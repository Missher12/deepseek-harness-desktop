...
- Version 0.1.7 adds the local Usage dashboard and passes 74 focused source tests plus the isolated packaged macOS smoke. Desktop staging validates 62 required files. The unsigned Intel DMG is 163,330,229 bytes with SHA-256 `481cd58ab9e69ad2eed42f6128be7fbf1b69d9352036102ef8d1107e9a3e1d27`; `hdiutil verify` passes, the bundle reports `0.1.7`, and the app executable plus packaged `node-pty` module report `x86_64`. It has not been installed over the existing application and no Windows build was run from macOS.
