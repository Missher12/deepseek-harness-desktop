...
- Model-control polish complete on 2026-08-17: the existing composer menu now contains a Claude Code-inspired wide DeepSeek-blue WebGL energy landscape with Faster/Smarter guidance, a thin drag track, sparse particles, and a static reduced-motion/WebGL fallback. It renders only Host-advertised stops, previews continuously while dragging, commits only on release, preserves exact effort ids, and aliases only `max` to `ULTRACODE`.
...
- Browser-level UI acceptance passed without a real API key or model request: the market rendered its discovery/search/install UI, the DeepSeek-V4-Flash menu exposed only `Off`, `High`, and `ULTRACODE`, keyboard `End` selected the exact `max` id, and the only console diagnostic was Chromium's pre-existing verbose password-form hint.
...
- Version 0.1.3 is installed at `/Applications/DeepSeek Harness.app`; the replaced application is recoverably retained as `~/Library/Application Support/DeepSeek Harness Backups/DeepSeek Harness-0.1.2-pre-0.1.3-20260817-1540.app`. Live startup completed on random loopback port 62047, the effort panel rendered through WebGL2 without console errors, and the settings market loaded 1,169 current registry entries. Pre/post hashes of existing session, workspace, and project-cache records were identical.
- The separately installed `dsh-effort-slider` package was removed after a recoverable profile backup at `~/Library/Application Support/DeepSeek Harness Backups/web-profile-pre-built-in-effort-20260817-1519`; the built-in panel replaces it without changing `~/.dsh` sessions. The shared macOS/Windows child launch now enables Electron's internal ESM-loader fallback so profile-installed Host plugins can resolve correctly.
