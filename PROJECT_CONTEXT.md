...
- Version 0.1.8 closes the residual generic-CI portability, exact-coverage, persistence-race, and usage-overflow contract gaps without changing the Desktop feature scope. The complete local coverage/static gates, Intel bundle checks, `hdiutil verify`, isolated packaged smoke, install-over-existing-app check, random-port launch, single-Harness-child check, and exact `~/.dsh` preservation check pass. Native Windows Setup remains gated by the final-commit Windows workflow and its real per-user install, shortcut, launch, close, process-cleanup, uninstall, and data-preservation acceptance. Exact release bytes are intentionally kept out of packaged documentation; the public Release assets and their matching ASCII/LF `.sha256` files are authoritative.
