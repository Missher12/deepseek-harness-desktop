...
## Current integration: Desktop 0.5.8 / official core 0.1.5-rc.2

The authorized upgrade is in progress on `codex/desktop-0.5.8-commander`. The last accepted public Desktop is 0.5.7 at `7f544cd31c43f239c09c3a209ae0d05fe677c710`. The new official source is `fb2c4b9e698e30edb738bca4cf0618587db7d203` (dsh-v0.1.5-rc.2). Content integration uses the verified previously imported `d347e703908d0406b7a7ef80e3a0e594d86b2215`; the older graph merge base is not the previously shipped core. The core merge retains both real parents in checkpoint `292fa7f4e7f762b2838403cee638ef8e24c0c4e1`. The follow-up native acceptance wiring and no-project navigation race fix remain under final validation.

Keep the existing Desktop shell, settings, update ownership, disabled Workbench and removed plugins. New default browser Sidebar services are disabled only by the Desktop overlay; explicit native file opening stays available. Preserve statistics placeholders/current-session state, bounded reasoning cards, final-answer folding, piano navigation, model selection, attachment limits and custom session-messenger history. Do not redesign context-window controls, enroll signing, change model settings or migrate the live home in this upgrade.

The core introduces current Session format V3, structured `SessionHandle.read()` results, lifecycle-owned Inbox/projections, managed subprocess runners and `native/system` POSIX flock payloads. Native package inventories and the old fs-ext CI probes must follow these actual interfaces. Historical generations remain immutable; validate supported historical Desktop records through the production migration pipeline before accepting a package. This source state has no final build SHA, no 0.5.8 native acceptance and no published 0.5.8 assets.

The user added persistent no-project Session directories to 0.5.8 on 2026-09-12. Desktop configures the shared Session Controller with `noProjectDirectory: deepseek-temp`; new Sessions without explicit project/cwd use `~/deepseek-temp/<sessionId>/`, retain that cwd on reopening, and preserve outputs on history deletion. Existing Session locations stay recorded. Native checks use isolated user homes. The authorized replacement Mac task is `01a0912e-3abe-75e1-8544-3a438c84039e`, worktree `.worktrees/desktop-058-mac-replacement`; the old Mac task is no longer an execution target.

Shared validation is complete: the supported full build, built-runtime smokes, keyless recorded-session replay, documentation and hygiene gates passed after focused repairs. Browser failures are closed through reviewed expected-output updates and targeted read-only replays; the new no-project flow also passed through the real composition. The native preflight subsequently identified a Workspace-membership ordering race: a project blank could be selected as no-project before its membership snapshot arrived. Reuse now excludes registered Workspace paths. The same five-write/V2 migration scenario runs in Chromium rehearsal and every packaged smoke, with one portable-receipt validator. Remaining work is the final follow-up commit and native package acceptance, followed by authorized publication. These source and browser results do not establish native installer acceptance.

The shared taskbook is `../../.artifacts/desktop-0.5.8-task.md` relative to this worktree. Current evidence and command results are in `.artifacts/desktop-058-release-state.json`; this ledger supersedes the historical checkpoints below. The fixed Mac, Windows and Ubuntu tasks retain native ownership, and all final packages must bind to the same final SHA. No new polling automation is permitted.
...
The real Windows updater handoff acceptance entry is integrated from `73ba2f72f07ecd61ea4467b5ba005917cd180fa8`. It starts an isolated installed application, runs the production system helper against the actual same-build Setup, waits for normal application exit, observes the helper-owned visible wizard, performs controlled cancellation and checks protected bytes and owned-process cleanup. Its final focused matrix passed 96 tests with seven native-only skips on macOS; host/Desktop types and typed lint passed. The entry runs within the existing Windows install/uninstall lifecycle. This native-command test does not claim a future public-version upgrade from the UI. Mac completed a read-only compatibility review of the integrated product code without blockers. The commander freezes the common source, obtains all platform ACKs, verifies Ubuntu 22.04/24.04 first, then builds and accepts Mac/Windows at the same SHA. Final Mac ten-sample fresh startup measurement remains required; no general 0.5.7 startup speedup is established.
