...
- Desktop integration for `@deepseek-ai/dsh-session-messenger` is complete: the immutable Desktop overlay mounts one canonical row while ordinary Web remains unchanged; automated real-Loader coverage proves native and Code Mode SDK tools, five exact same-origin Host routes plus index bootstrap, Client header/drawer composition, complete unload, disposed waits, and retention of committed inbox events. Plugin relays use a trusted metadata block plus a separate untrusted body block and render as visible conversation cards. The 320–560 px remembered drawer sends and replies from the current ordinary session, preserves failed drafts, copies the exact Session ID, and exposes metadata-only activity; reply authority stays Host-only. Package and staging preflights cover the published patch plus Host/Client artifacts. The four bounded tools implement send-without-wake, wake follow-up, receipt-bound reply, and matching-reply wait; they never create an automatic Agent loop.
