...
- The 2026-08-18 source candidate keeps the default-off ordinary reasoning thumb fully inside both track endpoints and reflows the embedded Plugin Market from its real Settings container width. Categories stay one-line and horizontally scrollable, search fills the toolbar, and narrow plugin actions occupy a second row. The pinned market Client artifacts were rebuilt from upstream `v1.10.1`, Desktop staging validated 62 files, and focused native Intel application geometry passed at a 564-pixel market width.
- This candidate's complete packaged smoke stops before reasoning and market acceptance because marking session notifications read appends to the selected session log, violating the smoke's no-session-mutation guarantee. The focused native UI acceptance remains valid for the two layout fixes; no replacement DMG, public release, or live `~/.dsh` installation has been produced from this branch.
...
- Desktop integration for `@deepseek-ai/dsh-session-messenger` is complete: the immutable Desktop overlay mounts one canonical row while ordinary Web remains unchanged; automated real-Loader coverage proves native and Code Mode SDK tools, Host routes/index bootstrap, Client graph/footer composition, complete unload, disposed waits, and retention of committed inbox events. Package and staging preflights cover the published patch plus Host/Client artifacts. Native packaged acceptance proves pending/unread notification state, mark-read, exact current Session-ID copy, and no unintended session selection or file mutation. The four bounded tools implement send-without-wake, wake follow-up, receipt-bound reply, and matching-reply wait; they never create an automatic Agent loop.
...
- Native Intel macOS 0.1.5 packaged UI acceptance passed without an API key or model request: exact ordinary and archived clipboard IDs, no-side-effect rejection paths, session-messenger notification/read/copy behavior, reasoning slider/Canvas/persistence, Plugin Market rendering/self-protection/ordinary uninstall, random-port ownership, and full process cleanup all passed.
- Version 0.1.5 passed full lint, 267 executed tests across 34 passing test files (one file and one test skipped by design), staging of 62 required files, and the isolated native packaged smoke. The unsigned Intel DMG is 163,338,960 bytes with SHA-256 `f04df6f48f868384edd1cd855429a2c5f43a059c7e63bd181e09297f38532422`; `hdiutil verify` passed, and the app executable plus packaged native modules report `x86_64` while the bundle reports `0.1.5`. Native Windows 0.1.5 packaging and acceptance are deferred; the pre-aligned workflow filenames are not Windows release evidence.
...
- The packaged smoke currently exposes a session-messenger mark-read side effect on the selected session log. Release promotion remains blocked until that mutation is diagnosed and either removed or explicitly reconciled with the no-session-mutation contract.
