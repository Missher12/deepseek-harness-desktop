...
- The audited marketplace patch now provides a compact Harness-native Discover list and stable Discover/Installed/Updates/Activity tabs, rejects disable/uninstall/update against both active-market aliases before package execution, and is guarded by source/bundle/source-map/Host coherence plus exactly-one-package staging checks. Repository and packaged smoke wiring is complete; real Intel macOS light/dark/200%-zoom visual acceptance remains a release gate.
- Desktop source integration for `@deepseek-ai/dsh-session-messenger` is complete: the immutable Desktop overlay mounts one canonical row while ordinary Web remains unchanged; automated real-Loader coverage proves native and Code Mode SDK tools, Host routes/index bootstrap, Client graph/footer composition, complete unload, disposed waits, and retention of committed inbox events. Package and staging preflights cover the published patch plus Host/Client artifacts. A real two-session native macOS Desktop exchange remains pending; this source change did not package, publish, install, or operate the live application.
...
- The Desktop source manifest and CI artifact paths now target version 0.1.5. The Intel macOS 0.1.5 application and DMG have not yet been built or verified, so the 0.1.4 artifact below remains the latest verified Mac evidence. Native Windows 0.1.5 packaging and acceptance are deferred; the pre-aligned workflow filenames are not Windows release evidence.
