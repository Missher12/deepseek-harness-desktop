...
Ubuntu preflight run 34254276529 built both packages and opened the installed application on 22.04 and 24.04; native checks then rejected Chromium's space-joined renderer process title. The Ubuntu owner fixed the title recognition without weakening kernel isolation checks and launched run 34255861928 at 7a763ae1d3958669710fc07360b856944bc6f89b. Both fixes are integrated. This run then passed kernel isolation and WM_CLASS checks on both OS versions, but the shared smoke used macOS's lsof path on Linux. Shared fix 0ac745435a68c00c33d131e7791824eb34911052 selects /usr/bin/lsof on Linux; the owner will supply an explicit lsof test dependency and repeat native preflight. The full native matrix is pending, and these preflight packages still contain 0.5.6 core, not the final shared 0.5.7 implementation. Ubuntu 24.04 AppImage needs the exact-path AppArmor helper; .deb integrates its own profile. Keep 0.5.6 available and publish all three platforms together only after one final SHA passes every native gate. The Windows installation progress fix and measured multi-scenario startup optimization remain pending with their owners.
