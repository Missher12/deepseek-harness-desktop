...
`core` crate 负责单调时钟租约过期、精确的会话/租约/版本/目标检查、配额，以及平台无关的有界辅助功能投影。投影按广度优先遍历，并固定在深度 32、2,000 个原始节点、300 个 ref、49,152 字节语义文本、128 字节 role 和 1,024 字节 name 内；它会跳过隐藏、最小化和窗口外节点。Ref 把快照版本绑定到进程内原生节点身份，而不是遍历位置，因此动态兄弟节点重排不能把动作重定向到别处；重复原生身份会失败关闭。平台接缝中不存在可编辑值字段。
...
可编辑焦点同时使用节点自身的 `AXFocused` 事实和应用权威的 `AXFocusedUIElement`，因为浏览器辅助功能树可能不提供子节点布尔值，但仍会报告准确聚焦的地址栏。Helper 只比较 AX 身份，并继续要求新鲜、可编辑且非敏感的 ref 才允许输入文字；它不会读取字段值，也不会根据坐标推断焦点。在 helper 自己执行多事件 Unicode 输入期间，同一原生身份的辅助功能名称或几何可以因这次输入而合理变化，但每个事件前仍会重新验证 role、可编辑性、敏感性、焦点、目标身份和其余全部准入事实。
