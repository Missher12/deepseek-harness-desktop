...
可编辑焦点同时使用节点自身的 `AXFocused` 事实和应用权威的 `AXFocusedUIElement`，因为浏览器辅助功能树可能不提供子节点布尔值，但仍会报告准确聚焦的地址栏。Helper 只比较 AX 身份，并继续要求新鲜、可编辑且非敏感的 ref 才允许输入文字；它不会读取字段值，也不会根据坐标推断焦点。
