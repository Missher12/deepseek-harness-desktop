...
The `core` crate owns monotonic lease expiry, exact session/lease/revision/target checks, quotas, and the platform-neutral bounded accessibility projection. That projection is breadth-first and stops at depth 32, 2,000 raw nodes, 300 refs, 49,152 semantic-text bytes, 128 role bytes, and 1,024 name bytes. It skips hidden, minimized, and off-window nodes. Refs bind the snapshot revision to a process-local native-node identity instead of a traversal position, so dynamic sibling reordering cannot retarget an action; duplicate native identities fail closed. Its platform seam has no editable-value field.
...
Editable focus uses both the node's `AXFocused` fact and the application's authoritative `AXFocusedUIElement`, because browser accessibility trees may omit the child boolean while still reporting the exact focused address field. The helper compares only AX identities and continues to require a fresh editable, non-sensitive ref before text entry; it does not read the field value or infer focus from coordinates. During the helper's own multi-event Unicode delivery, that same native identity may legitimately change its accessible name or geometry, while role, editability, sensitivity, focus, target identity, and every other admission fact are still revalidated before each event.
