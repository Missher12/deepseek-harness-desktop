...
| `ctx.browserControl` | `seam` | [`browser-control`](../packages/control/browser-control) | - | - | - | Owns internal lease acquisition, bounded snapshots, the exact ten-action roster, owner-bound opaque refs, and per-session surface revocation; later adapters provide the implementation. |
| `ctx.computerControl` | `seam` | [`computer-control`](../packages/control/computer-control) | - | - | - | Owns internal lease acquisition, bounded native observation, the exact eight-action roster, owner-bound opaque refs, fail-closed policy, and per-session Stop; later adapters provide the implementation. |
