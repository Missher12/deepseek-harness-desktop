...
| `ctx.missherBrain` | `core` | `missher-brain` | - | - | - | External Memory and Evolution bundles register bounded contribution providers; the hub owns the only automatic injection path, shared deadline, total byte budget, and pathless Settings snapshot. |
