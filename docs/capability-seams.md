...
| `ctx.browserControl` | `seam` | [`browser-control`](../packages/control/browser-control) | [`desktop-control-host`](../packages/control/desktop-control-host) | [`tool-browser-control`](../packages/control/tool-browser-control) | - | The service exposes only the closed semantic action roster; Electron owns lease authority, visible surface lifetime, and the provider implementation. |
| `ctx.computerControl` | `seam` | [`computer-control`](../packages/control/computer-control) | [`desktop-control-host`](../packages/control/desktop-control-host) | [`tool-computer-control`](../packages/control/tool-computer-control) | - | The service exposes the closed native-observation and input roster; the Desktop authority and native helper own approval, target identity, and execution. |
| `ctx.desktopProfiles` | `core` | `desktop-plugin-runtime` | - | - | - | Publishes the immutable active Desktop profile to the trusted plugin manager without exposing it through a renderer-supplied authority field. |
| `ctx.desktopPnpm` | `core` | `desktop-plugin-runtime` | - | - | - | Serializes trusted plugin package operations through the packaged pnpm entry and managed subprocess boundary. |
| `ctx.missherBrain` | `core` | [`missher-brain`](../packages/brain/missher-brain) | - | `ui-settings-brain` | - | Registers independent local providers, arbitrates bounded untrusted context, and exposes pathless provider status through its generated Remote. |
| `ctx.usageInsights` | `core` | [`usage-insights`](../packages/session/usage-insights) | - | `ui-settings-usage` | - | Folds durable Session accounting into a bounded client-facing snapshot without adding model context. |
