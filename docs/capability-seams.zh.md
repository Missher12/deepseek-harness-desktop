...
| `ctx.browserControl` | `seam` | [`browser-control`](../packages/control/browser-control) | - | - | - | 拥有内部租约获取、有界快照、精确十种动作清单、绑定所有者的 opaque ref 与逐会话 surface 撤销；后续适配器提供实现。 |
| `ctx.computerControl` | `seam` | [`computer-control`](../packages/control/computer-control) | - | - | - | 拥有内部租约获取、有界原生观察、精确八种动作清单、绑定所有者的 opaque ref、遇疑即拒策略与逐会话 Stop；后续适配器提供实现。 |
