...
flowchart LR
  ...
  svc_browserControl["ctx.browserControl<br/>Semantic browser-control seam"]
  pkg_computer_control["computer-control"]
  svc_computerControl["ctx.computerControl<br/>Native computer-control seam"]
  pkg_llm["llm"]
...
| `ctx.browserControl` | `seam` | [`browser-control`](../packages/control/browser-control) | - | - | - | 拥有有界快照、精确十种动作清单、绑定所有者的 opaque ref 与逐会话 surface 撤销的稳定 Host 约定；后续适配器提供实现。 |
| `ctx.computerControl` | `seam` | [`computer-control`](../packages/control/computer-control) | - | - | - | 拥有有界原生观察、精确八种动作清单、绑定所有者的 opaque ref、遇疑即拒策略与逐会话 Stop 的稳定 Host 约定；后续适配器提供实现。 |
...
| `ctx.desktopProfiles` | `core` | `desktop-plugin-runtime` | - | `dshmarket` | - | 向受信任的 Desktop 包管理器发布唯一且不可变的当前 web profile 身份，不授予切换 profile 的权限。 |
| `ctx.desktopPnpm` | `core` | `desktop-plugin-runtime` | - | `dshmarket` | - | 通过内置 DSH CLI 与 pnpm 入口串行执行可取消的包操作，不向 renderer 提供通用 subprocess bridge。 |
