...
flowchart LR
  ...
  svc_browserControl["ctx.browserControl<br/>Desktop semantic browser-control seam"]
  ...
  svc_computerControl["ctx.computerControl<br/>Desktop native computer-control seam"]
  ...
  svc_desktopPnpm["ctx.desktopPnpm<br/>Packaged Desktop plugin operation runner"]
  pkg_missher_brain["missher-brain"]
  svc_missherBrain["ctx.missherBrain<br/>Bounded external-brain provider hub"]
  pkg_ui_settings_brain["ui-settings-brain"]
  pkg_usage_insights["usage-insights"]
  svc_usageInsights["ctx.usageInsights<br/>Privacy-minimal usage projection"]
  ...
  pkg_browser_control --> svc_browserControl
  ...
  pkg_computer_control --> svc_computerControl
  pkg_cordis_host_runner --> svc_cordisInspect
  pkg_cordis_host_runner --> svc_dynamicCordisRunner
  pkg_credentials --> svc_credentials
  pkg_credentials_local --> svc_credentials
  pkg_deepseek_llm_api_extensions --> svc_deepseekLlmApiExtensions
  pkg_desktop_control_host --> svc_browserControl
  pkg_desktop_control_host --> svc_computerControl
  ...
  pkg_usage_insights --> svc_usageInsights
  ...
  svc_browserControl --> pkg_tool_browser_control
  svc_clientModules --> pkg_client_hmr
  svc_codeRuntime --> pkg_tools
  svc_compaction --> pkg_compaction_basic
  svc_computerControl --> pkg_tool_computer_control
  ...
  svc_missherBrain --> pkg_ui_settings_brain
  ...
  svc_usageInsights --> pkg_ui_settings_usage
...
| `ctx.browserControl` | `seam` | [`browser-control`](../packages/control/browser-control) | [`desktop-control-host`](../packages/control/desktop-control-host) | [`tool-browser-control`](../packages/control/tool-browser-control) | - | 该服务只公开封闭的语义操作清单；Electron 负责 lease 权限、可见 surface 生命周期与 provider 实现。 |
| `ctx.computerControl` | `seam` | [`computer-control`](../packages/control/computer-control) | [`desktop-control-host`](../packages/control/desktop-control-host) | [`tool-computer-control`](../packages/control/tool-computer-control) | - | 该服务只公开封闭的原生观察与输入清单；Desktop 权限层和原生 helper 负责审批、目标身份与执行。 |
| `ctx.desktopProfiles` | `core` | `desktop-plugin-runtime` | - | - | - | 向受信任插件管理器发布不可变的活动 Desktop profile，不通过 renderer 提供的权限字段公开。 |
| `ctx.desktopPnpm` | `core` | `desktop-plugin-runtime` | - | - | - | 通过打包的 pnpm 入口与受管 subprocess 边界串行化可信插件包操作。 |
| `ctx.missherBrain` | `core` | [`missher-brain`](../packages/brain/missher-brain) | - | `ui-settings-brain` | - | 注册相互独立的本地 provider，仲裁有界的不可信上下文，并通过生成的 Remote 公开不含路径的 provider 状态。 |
| `ctx.usageInsights` | `core` | [`usage-insights`](../packages/session/usage-insights) | - | `ui-settings-usage` | - | 将持久 Session 计费折叠为有界的客户端快照，不增加模型上下文。 |
