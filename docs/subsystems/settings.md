...
Host service backing the generated `ctx.remote.settings` namespace. Provider reads use `redactSecrets: true`, so a `role('secret')` field cannot ride a response; personalization projects only its browser-safe managed block and never the Host path. Writes expose the settings service's merge, replacement, and path-addressed operations, and classify every refusal as `settings/conflict` or `settings/rejected` with the owner's message.
