...
- `@deepseek-ai/dsh-session-messenger` — requires `tools` · `storageDomain` · `workspaceRegistry` · `typert` · `agents` · `sessionPersistence` · `webServer` ([`packages/extensions/session-messenger/src/index.ts`](../packages/extensions/session-messenger/src/index.ts))
