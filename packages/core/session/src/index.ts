...
import type { CreateSessionOptions, EpochHeader, LogIntent, PrepareSessionOptions, RequestContext, SessionEvent, SessionEventMap, SessionEventType, SessionHeader, SessionId, SurfaceIntent, SurfaceEventType } from './types.ts'
...
export class Session {
   ...
   *   history). Non-surface types accept only optional `LogIntent`, which
   *   carries the explicit ignorable flag without surface metadata. Assistant
  ...
  append<T extends SessionEventType>(
    ...
    ...opts: T extends SurfaceEventType ? [opts: SurfaceIntent<T>] : [opts?: import('./types.ts').LogIntent]
  ): SessionEvent<T> {
...
export class SessionStore extends Service {
  ...
  async flush(session: Session): Promise<boolean> {
    ...
    // The async wrapper captures synchronous listener throws while preserving
    // their exact rejection values for this caller-owned failure boundary.
