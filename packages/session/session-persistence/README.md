...
| `delete(id): Promise<boolean>` | Permanently remove one cold session. Coordinator-backed implementations serialize behind retirement and same-id operations, reject while the Session is attached or a preparation is reserved, and return whether durable or tracked state existed. Attachments and project files are outside this ownership boundary. |
