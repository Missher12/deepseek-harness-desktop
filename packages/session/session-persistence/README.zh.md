...
`delete(id)` 删除调用方已授权且无写入所有者的一个 Session 的所有已存储代际，并返回它是否存在。活动写入方会使操作拒绝；provider 必须保留相邻 Session。删除授权归调用方负责，不由存储 provider 决定。

### 失败与恢复

当前构建无法忠实解读的存储日志会以方向感知的错误被拒绝，绝不错读。`SessionHandle` 只暴露当前逻辑 v2 记录；provider 必须在返回句柄前转换任何受支持的历史存储，随产品交付的 JSONL provider 会通过静态 catalog 迁移已发布 v0/v1。更高格式会要求操作者升级 harness。本构建不认识的事件类型会被拒绝，除非其信封标记为 `ignorable`；已提交前缀中的损坏以 `SessionPersistenceCorruptionError` 拒绝。
