...
- **Deletion is explicit and Session-scoped** — `delete(id)` acquires the same kernel-backed ownership lease as a writer, refuses local or foreign ownership, and removes the exact Session directory with every historical generation. Other Sessions remain intact; deleting only the newest generation would resurrect a predecessor.
