...
class JsonlSessionPersistence extends SessionPersistence {
  ...
  /** Remove the exact session directory while holding its local and kernel write claims. */
  async delete(id: SessionId): Promise<boolean> {
    ...
    try {
      ...
      if (await this.readGenerationHeader(selected, id) === undefined) return false
