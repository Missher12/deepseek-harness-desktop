...
- **Deletion is explicit, not a retention policy** — `delete(id)` removes the session row transactionally and lets the events foreign key cascade; automatic age/size pruning is not provided.
