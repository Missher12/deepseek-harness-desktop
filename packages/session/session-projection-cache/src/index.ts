...
export class SessionProjectionCache extends Service {
  ...
  /** Per-id write tails: lifecycle checkpoints must reach storage in trigger order. */
  private readonly writes = new Map<SessionId, Promise<void>>()
  /** Closed before teardown drains queued writes and the domain closes. */
  private writeAdmissionOpen = true
  ...
  async write(session: Session): Promise<void> {
    if (!this.writeAdmissionOpen) {
      throw new Error('session projection cache is disposing')
    }
    const rows = this.ctx.sessionProjections.checkpoint(session)
    this.markClean(session)
    // Start the durability barrier at the same boundary as the checkpoint.
    // Its outcome is settled into data immediately so a queued predecessor
    // cannot leave a rejection temporarily unobserved.
    const durability = this.ctx.sessions.get(session.id) === session
      ? this.ctx.sessions.flush(session).then(
        () => ({ ok: true as const }),
        (error: unknown) => ({ ok: false as const, error }),
      )
      : Promise.resolve({ ok: true as const })
    const identity = identityOf(session.header)
    const prior = this.writes.get(session.id) ?? Promise.resolve()
    const run = async (): Promise<void> => {
      const outcome = await durability
      if (!outcome.ok) throw outcome.error
      await this.put(session.id, identity, rows)
    }
    const next = prior.then(run, run)
    const tail = next.then(() => undefined, () => undefined)
    this.writes.set(session.id, tail)
    void tail.then(() => {
      if (this.writes.get(session.id) === tail) this.writes.delete(session.id)
    })
    await next
  }
  ...
  private installWritePath(): void {
    // Register the drain before listeners: reverse-order teardown removes
    // event admission first, then waits for every queued write, then the
    // domain-close effect registered during init releases the medium.
