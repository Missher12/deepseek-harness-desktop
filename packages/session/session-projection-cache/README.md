...
The cache captures each checkpoint before waiting for log durability and commits the same session's records in request order. A slow flush cannot overwrite a newer checkpoint or delay another session's flush. Disposal stops new writes and drains accepted checkpoints before closing their storage domain.
