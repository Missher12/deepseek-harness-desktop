...
describe('sessionFormatV1ToV2', () => {
  it('preserves Desktop relay identities and body across sequence remapping', () => {
    const source: SessionFormatArtifact = {
      header: { version: 1, id: 'relay', createdAt: 1, isSeeded: false, delegationDepth: 0 },
      ...
      events: [
        ...
        event('assistant/chunk', 3, 4, { turn: 1, step: 1, chunk: { type: 'finish', reason: { kind: 'stop' } } }),
        { ...event('assistant/message', 4, 5, { turn: 1, step: 1, message }), sourceEventSeqs: [2, 3], surfaceOp: 'append' },
        { ...event('session-messenger/outgoing', 5, 6, { deliveryId: 'delivery', targetSessionId: 'target',
          body: '{"seq":3}', status: 'delivered', wakeRequested: false }), ignorable: true },
      ],
    }
    const target = sessionFormatV1ToV2.migrate(source)
    expect(target.events.at(-1)).toEqual({ ...source.events.at(-1), seq: 3 })
  })
