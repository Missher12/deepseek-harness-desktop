...
export function assertReleasedPayloadSemantics(event: SessionFormatEvent, version: number): void {
  ...
  switch (event.type) {
    case 'agent-preset/selected':
      ...
      return
    case 'session-messenger/outgoing':
      for (const key of ['deliveryId', 'targetSessionId'] as const) nonEmptyString(data[key], `${label} ${key}`)
      for (const key of ['replyToDeliveryId', 'continuationOfDeliveryId'] as const) {
        if (data[key] !== undefined) nonEmptyString(data[key], `${label} ${key}`)
      }
      stringValue(data['body'], `${label} body`)
      literalValue(data['status'], ['delivered', 'delivery-recovery-pending'], `${label} status`)
      literalValue(data['wakeRequested'], [true, false], `${label} wakeRequested`)
      return
...
function pluginSourceValue(source: JsonRecord, label: string): void {
  if (source['plugin'] === 'dsh-session-messenger' && source['form'] === 'relay') {
    assertReleasedV0Keys(source,
      ['kind', 'plugin', 'form', 'senderSessionId', 'deliveryId', 'mode', 'bodyBlockIndex'], [], label)
    nonEmptyString(source['senderSessionId'], `${label} senderSessionId`)
    nonEmptyString(source['deliveryId'], `${label} deliveryId`)
    literalValue(source['mode'], ['inject', 'followup'], `${label} mode`)
    literalValue(source['bodyBlockIndex'], [1], `${label} bodyBlockIndex`)
    return
  }
...
function subagentDescriptorValue(data: JsonRecord, label: string): void {
  // Released v2 composition has no Session-sequence references. Preserve its
  // version verbatim for history; the subagent owner still fences cold resume.
