...
已发布的子 Agent descriptor v2 记录保留精确版本及经过校验的一次性或可继续组合。V2 不接受推理强度字段。迁移不会将其改标为 v3，也不会使其获得冷恢复资格；当前子 Agent 所有者仍检查精确版本。未知未来 descriptor 版本及无法识别的 v2 字段仍拒绝迁移。

Desktop 构建也冻结其 `session-messenger/outgoing` 载荷：投递与 Session 标识符为字符串，正文是字面文本，状态与唤醒意图具有封闭类型。它不含 Session 序号引用。即使信封标记可忽略，未知成员仍拒绝迁移。
