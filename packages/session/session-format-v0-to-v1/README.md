...
Released subagent descriptor v2 records retain their exact version and validated one-shot or continuable composition. V2 admits no reasoning-effort field. The migration does not relabel them as v3 or make them eligible for cold resume; the current subagent owner keeps its exact-version check. Unknown future descriptor versions and unrecognized v2 fields remain migration refusals.

The Desktop build also freezes its `session-messenger/outgoing` payload: delivery and Session identifiers are strings, body is literal text, and status and wake intent have closed types. It contains no Session-sequence references. Unknown members still refuse migration even when the envelope is ignorable.
