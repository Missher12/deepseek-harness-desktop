...
describe('released Session format v0 to v1', () => {
  it('preserves Desktop incoming relay metadata in inbox and user messages', () => {
    const header = { type: 'session', version: 0, id: 'incoming', createdAt: 1, delegationDepth: 0 }
    const source = { kind: 'plugin', plugin: 'dsh-session-messenger', form: 'relay',
      senderSessionId: 'sender', deliveryId: 'delivery', mode: 'inject', bodyBlockIndex: 1 }
    const message = { id: 'message', role: 'user', source,
      content: [{ type: 'text', text: 'metadata' }, { type: 'text', text: 'untrusted body' }] }
    const rows = [
    ...
    expect(sessionFormatV0ToV1.migrate(releasedV0SessionFormatCodec.decodeArtifact(header, rows)).events).toEqual(rows)
    for (const extra of [{ sourceEventSeq: 0 }, { mode: 'future' }, { bodyBlockIndex: 0 }, { senderSessionId: 2 }]) {
      const row = { ...rows[1], seq: 0, data: { ...message, source: { ...source, ...extra } } }
      expect(() => sessionFormatV0ToV1.migrate(releasedV0SessionFormatCodec.decodeArtifact(header, [row]))).toThrow()
    }
  })

  it('preserves the exact Desktop outgoing relay schema without interpreting its body', () => {
    const header = { type: 'session', version: 0, id: 'relay', createdAt: 1, delegationDepth: 0 }
    const event = {
      type: 'session-messenger/outgoing', seq: 0, time: 1, ignorable: true,
      data: { deliveryId: 'delivery', targetSessionId: 'target', body: '{"seq":99}',
        status: 'delivered', wakeRequested: false, replyToDeliveryId: 'reply' },
    }
    const source = releasedV0SessionFormatCodec.decodeArtifact(header, [event])
    expect(sessionFormatV0ToV1.migrate(source).events).toEqual([event])
    for (const extra of [{ sourceEventSeq: 0 }, { status: 'future' }, { wakeRequested: 'yes' }]) {
      expect(() => sessionFormatV0ToV1.migrate(releasedV0SessionFormatCodec.decodeArtifact(header,
