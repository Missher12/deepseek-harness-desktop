...
describe('released Session format v0 to v1', () => {
  it('preserves Desktop incoming relay metadata in inbox and user messages', () => {
    ...
    for (const extra of [{ sourceEventSeq: 0 }, { mode: 'future' }, { bodyBlockIndex: 0 }, { senderSessionId: 2 }]) {
      ...
      expect(() => restoreV0ToV1(header, [row])).toThrow()
    }
  })
  ...
  it('preserves the exact Desktop outgoing relay schema without interpreting its body', () => {
    ...
    const source = [event]
    expect(restoreV0ToV1(header, source).events).toEqual([event])
    for (const extra of [{ sourceEventSeq: 0 }, { status: 'future' }, { wakeRequested: 'yes' }]) {
      expect(() => restoreV0ToV1(header, [{ ...event, data: { ...event.data, ...extra } }])).toThrow()
    }
  })
