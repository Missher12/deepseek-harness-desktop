...
const cacheRecordSchema = z.object({
  ...
  revision: z.string(),
  createdAt: countSchema,
  seedLength: countSchema.nullable(),
  timeZone: z.string(),
  row: rowSchema,
}).strict()
...
export class UsageInsightsGateway extends TypertRemoteService {
  ...
  constructor(ctx: Context, public config: Config = {}) {
    ...
    ctx.on('session/event', (session: Session) => {
      this.dirty.add(String(session.id))
    })
    ctx.on('session/disposed', (session: Session) => {
      ...
      this.dirty.delete(String(session.id))
    })
  }
  ...
  private async refresh(): Promise<UsageInsightsSnapshot> {
    ...
    const results = await this.mapConcurrent(snapshots, 8, snapshot => this.rowFor(snapshot, timeZone))
  ...
  private async rowFor(snapshot: SessionPersistenceSnapshot, timeZone: string): Promise<RefreshResult> {
    const cached = cacheRecordSchema.safeParse(this.requireTable().get(snapshot.header.id))
    const revision = String(snapshot.revision)
    const seedLength = snapshot.header.seedLength ?? null
    if (cached.success
      && cached.data.revision === revision
      && cached.data.createdAt === snapshot.header.createdAt
      && cached.data.seedLength === seedLength
      && cached.data.timeZone === timeZone
      && !this.dirty.has(String(snapshot.header.id))) {
      return { row: cached.data.row }
    }
    try {
      const inspection = await this.ctx.sessionPersistence.inspect(snapshot.header.id)
      const row = foldSessionUsage(inspection.meta, inspection.events, timeZone)
      const live = (this.ctx.get('sessions') as { get(id: SessionId): Session | undefined } | undefined)
        ?.get(snapshot.header.id) !== undefined
      return {
        row,
        ...live ? {} : {
          cache: {
            ...
            seedLength: inspection.meta.seedLength ?? null,
            timeZone,
            row,
          },
        },
