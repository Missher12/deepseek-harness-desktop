---
description: "通过有界读取和按修订失效的缓存索引持久化 Session 用量。"
...
每个 Session 的统计计算复用一个日期格式器，只对计入统计的用户活动、工具调用和有效用量样本换算日期。文本与推理片段不分配日期格式器。持久化修订检查、缓存失效和提供方计量仍然是统计依据。

当前 v2 用量来自结算后的 Assistant 消息及其紧凑尝试流，也包含分别计费的重试。缓存 schema 3 替换旧计量记录。每次刷新最多打开两个只读句柄，即使读取失败也会关闭，并在读取前观察迁移后的修订号。未变化的代际可跨重启复用派生缓存；缓存不保留原始流文本。
