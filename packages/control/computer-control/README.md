...
- `freezeComputerList()` and `freezeComputerSnapshot()` detach and deeply freeze provider collections under the protocol limits. `assertComputerActionCount()` applies the service's 64-action per-turn ceiling; a native lease may be narrower.

## Fail-closed policy

`classifyControlPolicy()` returns only `ALLOW`, `APPROVAL_REQUIRED`, or `DENY`. Known secure text, passwords, one-time codes, payments, files, biometrics, password managers, keychains, OS privacy/security targets, installation/removal, destructive deletion, and download-execute targets are always denied. Unknown sensitivity or effect is denied. Targetless status/list requests use the explicit `not-applicable` class instead of inventing a target. Stop is approval-free on the matching surface even when no target classification exists, while a wrong-surface Stop is denied. Ordinary reads and ordinary local interaction on an authorized ephemeral/native surface are allowed; external side effects and any mutation of a persistent human browser require a separate Electron-native approval.
