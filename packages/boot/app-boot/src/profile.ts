...
import {
  existsSync, lstatSync, mkdirSync, readFileSync, readdirSync, readlinkSync, renameSync, statSync, symlinkSync,
...
/** Recoverable home-owned archive for proxies generated by packaged 0.4.x releases. */
...
interface LegacyModuleProxyManifest {
  name: string
  version: string
  private: true
  type: 'module'
...
/** Read only a byte-exact proxy emitted by the historical ensureModuleProxy generator. */
function readLegacyModuleProxy(link: string, packageName: string): LegacyModuleProxy | undefined {
  try {
    ...
    const proxyExports = parsed.exports
    ...
    if (!hasExactKeys(proxyExports, Object.keys(expectedExports))
      || Object.entries(expectedExports).some(([subpath, entry]) => proxyExports[subpath] !== entry)) return undefined

    const expectedManifest: LegacyModuleProxyManifest = {
      name: packageName,
...
/** Atomically move one proven legacy proxy into its same-home recovery area. */
function recoverLegacyModuleProxy(
  link: string,
  packageName: string,
  home: string,
  proxy: LegacyModuleProxy,
): void {
  ...
  try {
    ...
    // Another launch may have moved this exact directory first. Creation of
    // the destination symlink below decides whether that race was benign.
    if ((error as NodeJS.ErrnoException).code !== 'ENOENT') throw error
  }
}

/** Ensure `link` is a symlink to `target`, recovering a proven 0.4.x proxy; any other real entry throws. */
function ensureSymlink(link: string, target: string, packageName: string, home: string): void {
  let stat
  try {
    stat = lstatSync(link)
  } catch {
  ...
  if (stat !== undefined) {
    if (!stat.isSymbolicLink()) {
      const legacyProxy = stat.isDirectory() ? readLegacyModuleProxy(link, packageName) : undefined
      ...
      recoverLegacyModuleProxy(link, packageName, home, legacyProxy)
      stat = undefined
    }
    if (stat !== undefined) {
...
export function healProfilesModuleFallback(
  ...
  for (const [packageName, target] of links) {
    ...
    ensureSymlink(link, physicalTarget, packageName, home)
