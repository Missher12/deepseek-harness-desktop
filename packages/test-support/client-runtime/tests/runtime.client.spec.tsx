...
describe('fixture session face', () => {
  it('fail-loud stubs name the missing verb; supplied overrides run instead', async () => {
    ...
    expect(() => bare.revealHistorySeq()).toThrow(/revealHistorySeq is not stubbed/)
...
describe('workspaces action face', () => {
  it('records every IWorkspaces verb with inert defaults and honors stubs', async () => {
    ...
    await ws.restoreSession('s1' as SessionId)
    expect(ws.list.getSnapshot().archivedSessionIds).toEqual([])
    expect(ws.calls.map(c => c.method)).toEqual(
      ['create', 'create', 'pickDirectory', 'rename', 'delete', 'openPath', 'insertBefore', 'insertSessionBefore', 'archiveSession', 'restoreSession'])
    ...
    ws.stub('restoreSession', () => Promise.resolve())
    ...
    await ws.restoreSession('s2' as SessionId)
    expect(ws.list.getSnapshot().archivedSessionIds).toEqual([])
    await runtime.dispose()
  })
})
