...
type WorkspaceStub<Key extends WorkspaceAction> = (
  ...args: Parameters<Extract<IWorkspaces[Key], (...args: never[]) => unknown>>
) => ReturnType<Extract<IWorkspaces[Key], (...args: never[]) => unknown>>
...
export class TestWorkspaces implements IWorkspaces {
  ...
  /** Restore a session from the archive set (recorded). */
