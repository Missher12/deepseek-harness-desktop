...
`withFileLock` creates a `<filename>.lock` sibling with `wx`. `EEXIST` identifies contention directly; `EPERM` also indicates contention when a fresh `lstat` confirms the lock path exists. On Windows, a transient `EACCES`, `EBUSY`, or `EPERM` from that metadata probe permits a parent listing to prove the exact lock entry exists, without hiding an unrelated permission failure. The lock records its creator's PID and is removed by the holder in a `finally`; contention backs off exponentially and fails when the per-call `waitMs` deadline (default two seconds) passes.
