...
`withFileLock` 以 `wx` 创建 `<filename>.lock` 同级文件。`EEXIST` 直接表示竞争；一次新的 `lstat` 确认锁路径存在时，`EPERM` 也表示竞争。在 Windows 上，元数据探测返回临时性的 `EACCES`、`EBUSY` 或 `EPERM` 时，可使用父目录列表证明精确锁条目存在，不掩盖无关的权限故障。锁记录创建者的 PID，由持有者在 `finally` 中移除；竞争按指数退避，在每次调用声明的 `waitMs` 期限（默认两秒）过后失败。
