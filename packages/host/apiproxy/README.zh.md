...
`workspace.restoreSession` 会移除一个归档标记，同时保留会话原有的 Workspace 位置。`session.delete` 是不可撤销的配套操作，边界有意更窄：只接受已归档的普通会话；运行中或由外部所有者持有的实时 Agent 会被拒绝；由本网关拥有的空闲 Agent 会先 dispose，随后等待持久层退役，删除精确的持久会话，再清理所有 Workspace／归档引用。子代理所有的会话会被拒绝。项目文件、共享附件、设置和凭据均不属于该操作。
