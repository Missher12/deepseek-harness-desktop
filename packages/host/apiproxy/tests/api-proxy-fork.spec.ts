...
describe('sessions.fork', () => {
  ...
  it('forks immediately before a completed middle turn without mutating the source', async () => {
    const ctx = await composed()
    const source = liveAgent(ctx, 'session-before-middle', 3)
    const sourceBefore = [...source.events]
  ...
  it('forks before the first turn as an empty lineage child', async () => {
    const ctx = await composed()
  ...
  it('rejects before-turn fork while the source agent is running', async () => {
    const ctx = await composed()
    const source = liveAgent(ctx, 'session-before-running', 1)
    const agent = ctx.agents.get(source.id)
    if (agent === undefined) throw new Error('missing source agent')
    ...
    if (!response.result.ok) expect(response.result.error.message).toMatch(/running/)
    await ctx.fiber.dispose()
  })

  it('rejects before-turn fork from a steering message inside the turn', async () => {
    const ctx = await composed()
    const source = ctx.sessions.create(sid('session-before-steering'), { meta: { cwd: '/proj' } })
    ...
    if (!response.result.ok) expect(response.result.error.message).toMatch(/opening user message/)
    await ctx.fiber.dispose()
  })

  it('carries the target turn model selection into an empty before-turn child', async () => {
    const ctx = await composed()
    const source = ctx.sessions.create(sid('session-before-routed'), { meta: { cwd: '/proj' } })
    source.append('turn/start', { turn: 1 })
    source.append('user/message', createUserMessage({
      content: [{ type: 'text', text: 'reroute me' }],
      source: { kind: 'user' },
    }), { surfaceOp: 'append' })
    ...
    if (child === undefined) throw new Error('missing before-turn child')
    await child.ctx.systemPrompt.assemble()
