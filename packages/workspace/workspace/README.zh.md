...
- `ctx.workspaceRegistry.archiveSession(id)`／`restoreSession(id)`／`archivedSessionIds`：覆盖在 workspace 记账之上的注册表级全局归档集合：被归档的会话从各分组视图中消失，但其会话日志和 `sessionIds` 席位保持不变，因此恢复后回到相同位置。归档接受任何实时或已持久化的会话（无论已记账还是 Ungrouped），对已归档的 id 直接完成而不写入，并拒绝未知 id；恢复操作具有幂等性。在该字段出现之前写入的状态解析为一个空集合。
- `ctx.workspaceRegistry.purgeSession(id)`：从所有 Workspace 记账、归档集合及注册表的 header／路径缓存中移除一个已持久删除的会话 id。它本身绝不删除日志，并支持持久层提交删除后进行崩溃安全的幂等重试。
