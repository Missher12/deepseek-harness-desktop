...
### Registry operations at a glance
...
- `ctx.workspaceRegistry.archiveSession(id)` / `restoreSession(id)` / `archivedSessionIds` — the registry-global archive set, layered over workspace accounting: an archived session disappears from grouping surfaces but keeps its session log and its `sessionIds` slot, so restore returns it to the same position. Archiving accepts any live or persisted session (accounted or Ungrouped), resolves without writing for an already archived id, and rejects an unknown id; restore is idempotent. State written before the field existed parses with an empty set.
- `ctx.workspaceRegistry.purgeSession(id)` — removes a durably deleted session id from every Workspace account, the archive set, and the registry's header/path caches. It never deletes a log itself and is idempotent for crash-safe retry after the persistence owner commits deletion.
