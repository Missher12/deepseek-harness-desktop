...
function renderedText(content: readonly ContentBlock[]) {
  return content.map((block) => {
    if (block.type !== 'text') throw new Error('shell fixture returned a non-text block')
    return block
  })
}
  ...
  try {
    ...
    if (nested) {
      ...
      block = {
        ...
        content: renderedText(event.data.content), isError: event.data.isError, subCalls: [],
      }
    } else {
      ...
      block = {
        ...
        content: renderedText(result.content), isError: result.isError, subCalls: [],
      }
    }
