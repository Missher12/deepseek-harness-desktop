...
Configure provider credentials and model lists, including custom pi-ai routes, from the Models page. First-run dialogs introduce the product and request DeepSeek credentials when needed. The editor preserves drafts, reports discovery and validation failures, and keeps credential values write-only. Host-owned settings and credential references remain authoritative.
...
Leaving a known model ID field fills missing local catalog defaults without sending credentials or calling the provider. Manual edits win. Changing the ID clears only values this editor filled automatically, and a late lookup cannot change an edited or removed row. Presets remain drafts until Apply; unknown models stay manually editable.
