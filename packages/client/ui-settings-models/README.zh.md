--- description: "dsh Web 客户端的模型设置与产品引导插件：提供方行、API 密钥管理、模型列表与 DeepSeek 首次运行弹窗。" kind: "package-reference" ---
...
pi-ai profile 的 `models` 列表就在卡片上编辑：一行一个模型，行上显示 id 与显示名称，上下文窗口、输出上限、逐模型推理强度上限与输入能力收在该行的展开区内，右侧是两个无文字的操作——展开与删除。上限会把 `reasoningEfforts` 具体化为直到所选最大值的标准档位，因此 Desktop 的六档控件可以显示 Ultra，同时只下发该模型已声明的真实档位。输入能力使用三态选择：自动会省略 `input`，仅文本写入 `[text]`，文本和图片写入 `[text, image]`；端点发现只有在响应明确报告支持模态时才会预填该选择，绝不根据 id 或显示名称猜测。空列表意味着「使用该路由的内置 catalog」，因此每一行都只会被刻意添加；清空容量会丢弃它，而不是存入一个 schema 会拒绝的值，配置留空的部分由适配器的路由级回退值定尺寸——留空的容量以这些回退值的量级作为占位符，那只是提示而非镜像：该字段按 1000 计 `K`，且部署可以覆盖这些回退值。不是正整数的容量根本不会被存下。
