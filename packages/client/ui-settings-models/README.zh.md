...
只接受 High 的端点使用独立的**仅 High**选项，它只写入 `{ high: "high" }`；该选项刻意区别于代表 Low 至 High 连续能力的 **High 上限**，从而避免为仅接受 High 的模型凭空声明较低 wire 值。Desktop 六档滑块仍可完整显示，因为每个视觉档位都会安全映射到这一项已公布的真实值。
