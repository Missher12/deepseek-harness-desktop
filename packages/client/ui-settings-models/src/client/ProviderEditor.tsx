 ...
 * same reason).
 * Reasoning effort is deliberately absent at provider scope: it is a
 * per-MODEL capability, so each pi-ai model row owns its explicit ceiling and
 * the composer's model picker offers the resulting levels. Everything else
 * stays owned by `settings.yaml`. Profile edits land as minimal `settings.mutate`
