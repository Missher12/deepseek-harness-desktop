...
import type { LatestTurnBillingProjection, TokenUsageProjection } from '@deepseek-ai/dsh-token-meter/client'
...
export const StatsLine = memo(function StatsLine({ useChat, useProjection, t }: StatsLineProps) {
  ...
  const model = billingModel?.kind === 'single' && billingModel.provider === 'deepseek-official'
    ? billingModel.model
    : undefined
  // One wall-clock snapshot owns both price lookup and the visible tier. The
  // aligned minute tick keeps documented Beijing boundaries self-updating.
