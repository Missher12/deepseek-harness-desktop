...
import { PromptRail, type PromptRailAnchor } from './PromptRail.tsx'
...
export function ChatView({
  ...
  const promptAnchors = useMemo<readonly PromptRailAnchor[]>(() => (
    turnNavigationItems.map(item => ({
      seq: item.turn,
      turn: item.turn,
      time: timeline.turns.get(item.turn)?.start?.time ?? 0,
      kind: 'turn-opening',
      preview: item.prompt || item.response,
    }))
  ), [timeline, turnNavigationItems])
  const activatePrompt = useCallback((turn: number): void => {
    const item = turnNavigationItems.find(candidate => candidate.turn === turn)
    if (item !== undefined) navigateToTurn(item)
  }, [navigateToTurn, turnNavigationItems])
