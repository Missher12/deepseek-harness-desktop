...
import { isSessionMessengerRelay, RelayNodeView } from './RelayNodeView.tsx'
...
type PresentedAttachment =
  ...
  | { readonly type: 'file'; readonly file: UserFile['attachment'] | UserDocument['attachment'] }
...
function contentParts(content: readonly unknown[]): {
  ...
  for (const block of content) {
    ...
    if (b.type === 'text' && typeof b.text === 'string') texts.push(b.text)
      ...
      attachments.push({ type: 'file', file: (b as UserDocument).attachment })
    }
...
export const ContextMessageNodeView = memo(function ContextMessageNodeView({ node, useSessions, t }: ChatNodeViewProps<'context'>) {
  const data = node.data
  if (isSessionMessengerRelay(data.source)) {
    return <RelayNodeView content={data.content} source={data.source} useSessions={useSessions} t={t} />
  }
  return (
    <ContextInjectionRow
      content={data.content}
