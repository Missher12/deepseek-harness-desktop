...
/** Prompt-shaped view of one loaded official Chat turn. */
export interface PromptRailAnchor {
  /** Official Turn number; retained as seq for the rail's stable identity. */
  readonly seq: number
  readonly turn: number
  /** Logged Turn start time when available. */
  readonly time: number
  /** Loaded navigation currently exposes one opening prompt per Turn. */
  readonly kind: 'turn-opening' | 'steering'
...
function visiblePromptAnchors(
  anchors: readonly PromptRailAnchor[],
  activeSeq: number | null,
): readonly PromptRailAnchor[] {
...
function samePromptAnchorIdentity(left: PromptRailAnchor, right: PromptRailAnchor): boolean {
...
function extendsPromptAnchorSet(
  previous: readonly PromptRailAnchor[],
  next: readonly PromptRailAnchor[],
): boolean {
...
export interface PromptRailProps {
  readonly anchors: readonly PromptRailAnchor[]
...
export const PromptRail = memo(function PromptRail({ anchors, activeSeq, onActivate, t }: PromptRailProps) {
  ...
  const previousAnchorsRef = useRef<readonly PromptRailAnchor[] | null>(null)
  ...
  const nearestAnchor = (clientY: number, track: HTMLElement): PromptRailAnchor | undefined => {
  ...
  const onMarkKeyDown = (event: KeyboardEvent<HTMLButtonElement>, anchor: PromptRailAnchor): void => {
          ...
          {visible.map((anchor, visibleIndex) => {
            ...
            return (
              <button
                ...
                aria-label={t('promptRail.jump', { index: anchor.turn, preview: label })}
