...
describe('ChatView', () => {
  it('leaves the turn rail unrendered when an unrelated Chat update commits', () => {
    ...
    const counting = ((key: string, vars?: Record<string, unknown>) => {
      if (key === 'promptRail.aria') railRenders += 1
  ...
  it('projects loaded turns into prompt and response navigation previews', () => {
    ...
    const h = makeHarness({}, {}, snapshot)
    const view = render(<h.ChatView {...h.props} />)
    const navigation = view.getByRole('navigation', { name: '过往发言' })
    expect(navigation.getAttribute('data-side')).toBe('left')
    const first = view.getByRole('button', { name: '转到第 1 条发言：first prompt' })
    const second = view.getByRole('button', { name: '转到第 2 条发言：second prompt' })
    expect(first.style.getPropertyValue('--dsh-prompt-position')).toBe('0%')
    expect(second.style.getPropertyValue('--dsh-prompt-position')).toBe('100%')
    expect(second.getAttribute('aria-current')).toBe('true')
    fireEvent.focus(first)
    const preview = view.getByRole('tooltip')
    expect(preview.textContent).toContain('first prompt')
    expect(preview.textContent).not.toContain('first response')
  })
  ...
  it('jumps to a turn anchor and reflows stable marks after an older page arrives', () => {
    ...
    const view = render(<h.ChatView {...h.props} />)
    const second = view.getByRole('button', { name: '转到第 2 条发言：second prompt' })
    expect(second.style.getPropertyValue('--dsh-prompt-position')).toBe('0%')
    ...
    const movedSecond = view.getByRole('button', { name: '转到第 2 条发言：second prompt' })
    expect(movedSecond).toBe(second)
    expect(movedSecond.style.getPropertyValue('--dsh-prompt-position')).toBe('50%')
  })
