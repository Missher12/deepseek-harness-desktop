...
**No project** reuses an ordinary blank Session only when it is outside registered Workspace membership and paths. This prevents a newly created project Session from being reused before its membership snapshot arrives; existing Session locations remain unchanged.
