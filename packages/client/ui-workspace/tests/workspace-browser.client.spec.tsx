...
describe('WorkspaceBrowser', () => {
  ...
  it('copies the exact session id from the flat-list row menu', async () => {
    const writeText = vi.fn(async () => {})
    const restoreClipboard = installClipboard(writeText)
    try {
      const b = mount({
        useSessions: hook(sessionState([summary('flat-session-id', 1, { displayTitle: '单列表会话' })])),
        useWorkspaces: hook(workspaceState([workspace('alpha', ['flat-session-id'])])),
      })
      act(() => { b.store.actions.setGroupBy('flat') })
      fireEvent.click(screen.getByRole('button', { name: '会话“单列表会话”的操作' }))
      fireEvent.click(screen.getByRole('menuitem', { name: '复制会话 ID' }))

      await waitFor(() => { expect(writeText).toHaveBeenCalledWith('flat-session-id') })
      await waitFor(() => { expect(screen.getByRole('alert').textContent).toBe('会话 ID 已复制') })
    } finally {
      restoreClipboard()
    }
