...
describe('UiWorkspaceService', () => {
  ...
  it.each(['new-navigation', 'dispose'] as const)('preserves a no-project draft after %s supersedes its pending create', async (supersession) => {
    const b = bench({ sessions: sessionState([summary('current')], sid('current')) })
    const created = Promise.withResolvers<SessionId>()
    b.sessions.create.mockReturnValue(created.promise)
    const moveDraft = vi.fn()
    const opening = b.uiWorkspace.openNoProject(moveDraft)
    if (supersession === 'dispose') await b.ctx.fiber.dispose()
    else b.layout.selectPanel('panel-a' as MainPanelId)
    created.resolve(sid('late-ungrouped'))
    await opening
    expect(moveDraft).not.toHaveBeenCalled()
    expect(b.sessions.open).not.toHaveBeenCalled()
  })

  it('opens a no-project Session only after its current draft transfer succeeds', async () => {
    const b = bench()
    const moveDraft = vi.fn()
    await b.uiWorkspace.openNoProject(moveDraft)
    const selected = b.sessions.open.mock.calls[0]![0]
    expect(moveDraft).toHaveBeenCalledExactlyOnceWith(selected)
    expect(moveDraft.mock.invocationCallOrder[0]).toBeLessThan(b.sessions.open.mock.invocationCallOrder[0]!)
    b.sessions.open.mockClear()
    await expect(b.uiWorkspace.openNoProject(() => { throw new Error('draft failed') })).rejects.toThrow('draft failed')
    expect(b.sessions.open).not.toHaveBeenCalled()
  })
