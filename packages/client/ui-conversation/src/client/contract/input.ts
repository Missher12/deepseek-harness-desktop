...
/** Presentation data shared with a programmatically composed trigger launcher. */
...
/** One registered source requested by a named programmatic launcher. */
export interface InputLauncherSource {
  readonly name: string
  /** Presentation-only candidate projection; pick routing remains source-owned. */
  readonly project?: (
    items: readonly InputLauncherCandidate[],
  ) => readonly InputLauncherCandidate[]
}
...
export interface InputTriggerController {
  ...
  toggleSources(launcher: string, sources: readonly InputLauncherSource[], hit: InputTriggerHit): void
}
