...
import { MAX_PROMPT_ATTACHMENT_BASE64_CODE_UNITS, promptAttachmentBase64CodeUnits } from '@deepseek-ai/dsh-attachment/types'
...
export class ConversationController extends Service implements IConversation {
  ...
  async sendSession(
    ...
    const pendingAttachments = attachments.filter(attachment => attachment.kind !== 'document').map(attachment => attachment.kind === 'image'
      ? {
        type: 'image' as const,
        value: {
    ...
    const serializeAttachments = () => this.serializeAttachments(attachments)
    ...
    return files.map((file) => {
      ...
      if (classification.kind === 'document') {
        const attachment: ComposerDocumentAttachment = {
          kind: 'document', id: randomUUID() as DraftAttachmentId, file, mediaType: classification.mediaType,
        }
        this.draftAttachments.set(attachment.id, attachment)
        return attachment
      }
  ...
  async serializeDraftAttachments(
    ...
    return {
      attachments: await Promise.all(attachments.map(async (attachment) => {
        if (attachment.kind === 'document') throw new Error('conversation.serializeDraftAttachments: slash commands do not accept document attachments')
  ...
  private async serializeAttachments(
    ...
    for (const attachment of attachments) {
      if (attachment.kind === 'image') {
        ...
        const upload = this.fileUploads.getSnapshot()[attachment.id]
        if (upload === undefined || upload.status !== 'ready') throw new Error('conversation.sendSession: file upload is not ready')
        serialized.push({ type: 'file', receiptId: upload.receiptId })
      }
    }
    return serialized
  }
