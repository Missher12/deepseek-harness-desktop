...
// The `imageLimits` projection key merge (intake pre-check) is declared by the
// API Session Controller and arrives through this package's direct Client
// contract imports.
...
export function InputBar({
  ...
  const menuLauncher = useMenuLauncher(source => source)
  const addMenuOpen = menuLauncher === 'composer-add'
  const programmaticMenuOpen = menuLauncher !== null
  ...
  const canSteerQueue = !locked && !machineBusy && !programmaticMenuOpen && empty && running && subagent === null
