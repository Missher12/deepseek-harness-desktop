...
export const InputBar = memo(function InputBar({
  ...
  useEffect(() => addFiles === undefined || sessionId === undefined
    ? undefined
    : bindComposerImagePicker(sessionId, () => { imagePickerRef.current?.click() }), [addFiles, sessionId])
  ...
  const canSteerQueue = !locked && !machineBusy && !addMenuOpen && empty && running && steeringAvailable
  ...
  const intakeFiles = useCallback((files: readonly File[]): void => {
    ...
    const rejected = ((): string | null => {
      ...
      const combined = [
        ...
        ...files.filter((_, index) => classified[index]?.kind !== 'file'),
      ]
  ...
  }, [subagent, addFiles, attachments, documentLimits, imageLimits, showToast, t])
  ...
  return (
    <div className={clsx(css.root, variant === 'hero' && css.hero)}>
      ...
      {variant === 'composer' && sessionId !== undefined
        ? <div className={css.composerDock} data-composer-dock="">
