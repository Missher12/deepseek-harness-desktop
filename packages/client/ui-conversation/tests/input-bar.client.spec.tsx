...
describe('Add launcher chrome and control seats', () => {
  ...
  it('passes the editor selection to the Add launcher and reflects its expanded state', () => {
  ...
  it('opens the hidden image chooser through Add and sends selected files through intake', () => {
    const addImages = vi.fn(() => null)
  ...
  it('the Access chip renders the projection value and submits a non-Full-access pick directly', async () => {
    ...
    expect(items.map(o => o.textContent)).toEqual(['仅可查看', '可写入工作区', '完全权限（可写工作区外）'])
  ...
  it('requires explicit risk acknowledgement before submitting full access', async () => {
    ...
    fireEvent.click(view.getByRole('menuitem', { name: '完全权限（可写工作区外）' }))
    ...
    expect((view.getByLabelText(/^访问模式/) as HTMLButtonElement).textContent).toBe('完全权限（可写工作区外）')
    await act(async () => {})
  })
