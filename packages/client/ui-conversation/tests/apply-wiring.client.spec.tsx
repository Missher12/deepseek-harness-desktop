...
async function bench(options: {
  ...
  if (options.registerSource !== undefined) {
    runtime.ctx.provide('inputTriggers', { registerSource: options.registerSource } as never)
  }
...
describe('target-neutral Conversation apply wiring', () => {
  ...
  it('registers and collects the optional composer Add source', async () => {
    const unregister = vi.fn()
    const registerSource = vi.fn((_source: unknown) => unregister)
    const b = await bench({ registerSource })
    expect(registerSource).toHaveBeenCalledOnce()
    expect(registerSource.mock.calls[0]?.[0]).toMatchObject({
      trigger: '/', name: 'composer-add', launcherOnly: true, showGroupTitle: false,
    })
