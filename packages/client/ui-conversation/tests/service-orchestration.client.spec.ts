...
describe('ConversationController', () => {
  ...
  it('classifies image MIME drafts as images and every other file as an uploading file draft', async () => {
    ...
    const uploadFile = vi.fn((_data: Blob | Uint8Array) => Promise.resolve({
      ...
      value: {
        ...
        file: { attachmentId: 'sha256:1' as never, name: 'notes.bin', bytes: 2 },
      },
    }))
    ...
    const drafts = b.root.createDrafts(session.sessionId, [
      ...
      new File([Uint8Array.of(2)], 'notes.bin', { type: 'application/pdf' }),
    ])
  ...
  it('serializes command files as staged receipts without reading their bytes again', async () => {
    ...
    const drafts = b.root.createDrafts(session.sessionId, [
      new File([Uint8Array.of(1)], 'one.bin', { type: 'text/plain' }),
      new File([Uint8Array.of(2)], 'two.bin', { type: 'text/plain' }),
    ])
    await vi.waitFor(() => {
    ...
    try {
      await expect(b.root.serializeDraftAttachments(drafts.map(draft => draft.id))).resolves.toEqual({
        attachments: [
          { type: 'file', receiptId: 'receipt-one.bin' },
          { type: 'file', receiptId: 'receipt-two.bin' },
        ],
      })
    } finally {
      vi.unstubAllGlobals()
  ...
  it('bounds upload Workers, advances on settlement, and skips a queued file removed by the user', async () => {
    ...
    const drafts = b.root.createDrafts(session.sessionId, ['one', 'two', 'three', 'four', 'removed'].map(name =>
      new File([Uint8Array.of(1)], `${name}.bin`, { type: 'text/plain' })))

    expect(uploadFile.mock.calls.map(call => call[1])).toEqual(['one.bin', 'two.bin'])
    ...
    expect(uploadFile.mock.calls.map(call => call[1])).not.toContain('removed.bin')
  ...
  it('cancels a superseded upload and stages the carried draft on the target Session', async () => {
    ...
    const [attachment] = b.root.createDrafts(source.sessionId, [
      new File([Uint8Array.of(4)], 'carry.bin', { type: 'application/pdf' }),
    ])
    if (attachment === undefined) throw new Error('file draft missing')
  ...
  it('cancels a file upload when its draft is removed', async () => {
    ...
    ;(session as { uploadFile?: unknown }).uploadFile = uploadFile
    const [attachment] = b.root.createDrafts(session.sessionId, [
      new File([Uint8Array.of(5)], 'removed.bin', { type: 'application/pdf' }),
    ])
    if (attachment === undefined) throw new Error('file draft missing')
    await vi.waitFor(() => { expect(uploadFile).toHaveBeenCalledOnce() })
  ...
  it('keeps the accepted file draft until its rpcId appears in the Host queue', async () => {
    ...
    const [attachment] = b.root.createDrafts(session.sessionId, [
      new File([Uint8Array.of(6)], 'sent.bin', { type: 'application/pdf' }),
    ])
    if (attachment === undefined) throw new Error('file draft missing')
    await vi.waitFor(() => {
      expect(b.root.fileUploads.getSnapshot()[attachment.id]?.status).toBe('ready')
  ...
  it('awaits an aborted upload before disposing the service', async () => {
    ...
    b.root.createDrafts(session.sessionId, [
      new File([Uint8Array.of(7)], 'dispose.bin', { type: 'application/pdf' }),
    ])
    ...
    try {
      ...
      ;(session as { uploadFile?: unknown }).uploadFile = vi.fn(() => Promise.resolve({
        ...
        value: {
          ...
          file: { attachmentId: 'mixed-file' as never, name: 'notes.bin', bytes: 1 },
        },
      }))
      const drafts = b.root.createDrafts(session.sessionId, [
        new File([Uint8Array.of(1)], 'first.png', { type: 'image/png' }),
        new File([Uint8Array.of(2)], 'notes.bin', { type: 'text/plain' }),
      ...
      expect(echo?.attachments.map(attachment => attachment.type === 'image'
        ...
        { type: 'file', value: { attachmentId: 'mixed-file', name: 'notes.bin', bytes: 1 } },
      ...
      b.retire.onRetire?.({
        ...
        attachments: [
          ...
          { attachmentId: 'mixed-file' as never, name: 'notes.bin', bytes: 1 },
          {
    ...
    try {
      ...
      ;(session as { uploadFile?: unknown }).uploadFile = vi.fn(() => Promise.resolve({
        ...
        value: {
          ...
          file: { attachmentId: 'kept-file' as never, name: 'kept.bin', bytes: 1 },
        },
      }))
      ...
      const attachments = b.root.createDrafts(session.sessionId, [
        ...
        new File([Uint8Array.of(8)], 'kept.bin', { type: 'text/plain' }),
      ])
      await vi.waitFor(() => {
...
describe('document compatibility', () => {
  it('keeps mixed image and document order and sends document bytes without a browser path', async () => {
    ...
    const attachments = b.root.createDrafts(b.runtime.sessions.binding('s1')!.session.sessionId, [image, document])
    ...
    await expect(sending).resolves.toEqual({ kind: 'success' })
    expect(b.root.resolveDraftAttachments(attachments.map(attachment => attachment.id))).toEqual([])
    created.mockRestore()
    await b.runtime.dispose()
  })
  ...
  it('rejects a category-valid mixed batch above the shared request carrier before reading any file', async () => {
    ...
    const attachments = b.root.createDrafts(b.runtime.sessions.binding('s1')!.session.sessionId, files)
  ...
  it('reads document attachments sequentially instead of retaining every raw file buffer at once', async () => {
    const b = await bench()
    const created = vi.spyOn(URL, 'createObjectURL').mockReturnValue('blob:image')
    const image = new File([Uint8Array.of(1)], 'first.md', { type: 'text/markdown' })
  ...
  it('accepts the shared extension and extensionless text-document roster', async () => {
    const b = await bench()
    const attachments = b.root.createDrafts(b.runtime.sessions.binding('s1')!.session.sessionId, [
