...
it.each(['opening', 'revealing'] as const)('keeps file opening available while the native action is %s', (phase) => {
  const p = props()
  ...
  fireEvent.click(view.getByRole('button', { name: 'Open file out/report.pdf' }))
  fireEvent.click(view.getByRole('button', { name: 'Open out/report.pdf' }))
...
it('keeps the native menu disabled until a desktop is available', () => {
  ...
  expect((view.getByRole('button', { name: 'More file actions for out/report.pdf' }) as HTMLButtonElement).disabled).toBe(true)
  expect((view.getByRole('button', { name: 'Open out/report.pdf' }) as HTMLButtonElement).disabled).toBe(false)
  view.rerender(<PresentedFileCard {...p} host={{ ...p.host, available: false, fileManager: null }} />)
  expect((view.getByRole('button', { name: 'More file actions for out/report.pdf' }) as HTMLButtonElement).disabled).toBe(true)
})

it('opens the file from either the card or its primary button', () => {
  const p = props()
  const view = render(<PresentedFileCard {...p} />)
  fireEvent.click(view.getByRole('button', { name: 'Open file out/report.pdf' }))
  fireEvent.click(view.getByRole('button', { name: 'Open out/report.pdf' }))
...
it('keeps focus on the available preview button after selecting a native action', () => {
  ...
  const preview = view.getByRole('button', { name: 'Open out/report.pdf' })
