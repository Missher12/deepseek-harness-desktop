...
function openProps(controller = new PresentedOpenController()) {
  ...
  return {
    ...
    previewPresented: vi.fn((id: SessionId, file: { seq: number; index: number }) => controller.open(id, file.seq, file.index)),
...
describe('presented files', () => {
  ...
  it('uses the viewed fork Session in every open action and expands all delivered files', () => {
    ...
    props.previewPresented.mockResolvedValue(undefined)
    ...
    fireEvent.click(view.getByRole('button', { name: 'Open file report-0.docx' }))
    fireEvent.click(view.getByRole('button', { name: 'Open report-0.docx' }))
    expect(props.previewPresented).toHaveBeenCalledTimes(2)
    expect(props.previewPresented).toHaveBeenLastCalledWith('child-session', expect.objectContaining({ path: 'report-0.docx', seq: 2, index: 0 }), undefined)
