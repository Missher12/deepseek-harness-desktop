  ...
  ctx.slots.inject(
    ...
    () => ctx.slots.register({
      ...
      inject: (): DeliverablesInjected => ({
        ...
        previewPresented: async (sessionId, file, cwd) => {
          const sidebar = ctx.get('sidebarRight')
          if (sidebar === undefined) await opener.open(sessionId, file.seq, file.index)
          else sidebar.openResource(fileAddressFor(sessionId, cwd, file.path))
        },
      }),
    }, Deliverables),
  )
