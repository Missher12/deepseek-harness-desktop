...
export interface DeliverablesInjected {
  ...
  previewPresented: (sessionId: SessionId, file: PresentedPath, cwd: string | undefined) => Promise<void>
}
 ...
 */
export function Deliverables({ matched, openFile, t, sessionId, useSessions, openPresented, previewPresented, usePresentedOpen, usePresentedHost, reloadPresentedHost }: Pick<TurnTailOwnerProps, 'openFile'> & {
    ...
    >
      ...
      <div className={css.presented} data-presented-files-row data-single={matched.presented.length === 1 ? true : undefined}>
        {presented.map(file => <PresentedFileCard key={`${file.seq}:${file.index}`} file={file} cwd={cwd}
          ...
          onPreview={() => { void previewPresented(sessionId, file, cwd) }}
