...
`Input` uses a light semantic border outline, a distinct focus ring, and stronger placeholder contrast in Desktop windows. The outline uses the theme's `border-l4` token and does not change layout when focused.
...
Tight top-level paragraph lists retain completed items and parse only their last item and appended content. Loose or nested lists, reference syntax, raw HTML, and following blocks return to the complete tail grammar immediately. Ordered-list starts and source positions survive the incremental path; the final render still performs the complete math-enabled parse.

While a reply streams, `MarkdownText` parses incrementally: all but the trailing two blocks freeze as cached React elements and only the source tail re-parses per chunk, so per-chunk work tracks the tail instead of the whole reply. A final unclosed top-level fence keeps its parsed code node and sends only the last completed line plus the current partial line through the same GFM grammar; a closing fence or ambiguous parse returns to the ordinary tail path. Highlighting likewise resumes from saved Shiki grammar state and publishes only newly completed lines plus the mutable tail. `CodeBlock` seals completed lines into fixed-size React groups, reuses earlier groups, and retains the whole highlighted tree across settlement when code and language are unchanged. The settled full parse still resolves references that crossed the freeze boundary ([incremental renderer](../../../.agents/notes/archived/architecture/2026-08-06-web-markdown-incremental-ast-renderer.md), [streaming fence highlighting](../../../.agents/notes/archived/feature/2026-08-20-web-streaming-fence-highlight.md)).
