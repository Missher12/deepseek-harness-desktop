...
describe('ComposerAttachments', () => {
  it('accepts file drops anywhere on the document and keeps non-file drags native', () => {
    ...
    const view = render(<ComposerAttachments {...props({
      onAddFiles,
      dropLimits: { images: { count: 20, size: '5MB' }, documents: { count: 5, size: '20MB' } },
    })} />)
...
function documentAttachment(id: string, name: string): ComposerAttachment {
  return { kind: 'document', id: id as ComposerAttachment['id'], file: new File(['hello'], name, { type: 'text/markdown' }), mediaType: 'text/markdown' }
}
...
describe('document compatibility', () => {
  it('renders documents as removable file cards without opening the image lightbox', () => {
    ...
    const view = render(<ComposerAttachments {...props({ attachments: [document], onRemoveAttachment })} />)
    ...
    expect(onRemoveAttachment).toHaveBeenCalledExactlyOnceWith(document.id)
  })
