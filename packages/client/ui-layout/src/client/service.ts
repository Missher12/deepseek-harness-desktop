...
import type { createLayoutStore, UtilityMode } from './stores.ts'
...
export interface ILayout {
  /**
   * Open the retained optional workbench state.
   * @param mode - selected surface.
   */
  openUtility(mode?: UtilityMode): void
  /** Close the retained optional workbench state. */
  closeUtility(): void
  /**
   * Toggle a retained surface.
   * @param mode - selected surface.
   */
  toggleUtility(mode?: UtilityMode): void
  /**
   * Set its bounded preference.
...
export class LayoutController implements ILayout {
   ...
   * @param mode - selected surface.
   */
  openUtility(mode?: UtilityMode): void { this.panels.openUtility(mode) }

  /** Close the retained optional workbench state. */
  closeUtility(): void { this.panels.closeUtility() }

  /**
   * Toggle a retained surface.
   * @param mode - selected surface.
   */
  toggleUtility(mode?: UtilityMode): void { this.panels.toggleUtility(mode) }

  /**
   * Set its bounded preference.
   * @param px - requested width in CSS pixels.
   */
  setUtilityWidth(px: number): void { this.panels.setUtilityWidth(px) }
