...
describe('createLayoutStore', () => {
  it('starts with the default sidebar and no right panel preference', () => {
    ...
    expect(store.getSnapshot()).toEqual({
      utilityOpen: false, utilityMode: 'terminal', utilityWidth: 360,
  ...
  it('retains source-only utility actions without changing visible panel geometry', () => {
    const { store, actions } = createLayoutStore().create()
    const layoutInfo = store.getSnapshot().layoutInfo
    actions.openUtility('files')
    expect(store.getSnapshot()).toMatchObject({ utilityOpen: true, utilityMode: 'files' })
    actions.setUtilityWidth(9999)
    expect(store.getSnapshot().utilityWidth).toBe(720)
    actions.setUtilityWidth(1)
    expect(store.getSnapshot().utilityWidth).toBe(300)
