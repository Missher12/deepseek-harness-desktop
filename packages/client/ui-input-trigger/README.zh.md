...
在输入框中键入 `/` 或 `@`，即可打开分组的命令和引用选项。键盘、指针与程序化入口共用同一选择流程，包括逐层展开的选项。选择后执行已注册命令或插入引用；本包不组装或发送模型请求。
...
细化查询尚未完成时，保留的候选项通过 `aria-disabled` 表明暂不可选。所在组就绪后才可选中；浏览器消费方应等待候选项启用后，再点击或按 Tab。
