...
export class InputTriggerController {
  ...
  /** Requested source order retained while a composed launcher stays open. */
  private launcherSourceNames: readonly string[] = []
  /** Presentation projections retained across live-source invalidations. */
  private launcherProjections: ReadonlyMap<string, LauncherSource['project']> = new Map()
   ...
   * @param hit - synthetic trigger hit carrying position and pick-time draft CAS.
   */
  toggleSources(launcher: string, requested: readonly LauncherSource[], hit: TriggerHit): void {
    ...
    this.stopFetch()
    this.hit = hit
    this.launcherSourceNames = matches.map(source => source.name)
    this.launcherProjections = projections
  ...
  private watchLexicon(source: InputTriggerSource, projection: ClientSessionContext): void {
    ...
    this.lexiconOffs.set(source, source.subscribeLexicon(projection, () => {
      ...
      void Promise.resolve().then(() => {
        ...
        const registered = new Map(this.deps.roster.sources(hit.trigger).map(item => [item.name, item]))
        const launched = this.launcher.getSnapshot() !== null
        const roster = launched
          ? this.launcherSourceNames.flatMap(name => registered.get(name) ?? [])
          : [...registered.values()].filter(item => item.launcherOnly !== true)
        this.fetchCandidates(hit, roster, launched ? this.launcherProjections : new Map())
      })
    }))
  }
  ...
  private clearLauncher(): void {
    ...
    this.launcherSourceNames = []
    this.launcherProjections = new Map()
  }
