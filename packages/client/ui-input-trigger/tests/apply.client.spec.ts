...
async function bench() {
  ...
  return { ctx, slots, locale, sessionScope: scope.ctx }
}
...
describe('apply', () => {
  ...
  it('registers the bilingual menu dictionaries (group titles by source name + the pending row)', async () => {
    ...
    expect(t('plugin')).toBe('插件')
    expect(t('cordis')).toBe('动态插件')
    locale.setLocale('en')
    expect(t('skill')).toBe('Skills')
    expect(t('plugin')).toBe('Plugins')
    expect(t('cordis')).toBe('Dynamic plugins')
  ...
  it('registers MenuView into the overlay and resolves the per-session controller by slot sessionId', async () => {
    const { ctx, slots, sessionScope } = await bench()
    await ctx.plugin({ inject: [...inject], apply }).await()
    ...
    const controller = inputTriggers.sessionOf(sessionScope)
