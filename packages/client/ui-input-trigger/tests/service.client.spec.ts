...
describe('programmatic source launcher', () => {
  ...
  it('keeps a composed launcher roster and projections when a source invalidates its lexicon', async () => {
    let invalidate = (): void => {}
    const add = readySource('/', 'composer-add', [{ name: 'files' }])
    const command = readySource('/', 'command', [{ name: 'goal' }])
    const skillBase = readySource('/', 'skill', [{ name: 'review' }])
    ...
    const { controller } = controllerBench([add.source, command.source, skill])
    const hit = {
      trigger: '/' as const,
      query: '',
      quoted: false,
    ...
    expect(controller.menu.getSnapshot().groups).toEqual([
      { source: 'composer-add', status: 'ready', items: [{ name: 'files', section: 'Add' }] },
      { source: 'skill', status: 'ready', items: [{ name: 'review', section: 'Plugins' }] },
    ])
  })
