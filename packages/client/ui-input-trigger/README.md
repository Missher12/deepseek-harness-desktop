...
Open grouped command and reference choices by typing `/` or `@` in the composer. Keyboard, pointer and programmatic launchers share the same selection flow, including drill-down choices. A selection invokes a registered command or inserts a reference; this package does not assemble or send model requests.
...
Retained candidates expose `aria-disabled` while a refined query is pending. They become selectable when their group is ready; browser consumers wait for an enabled option before clicking or pressing Tab.
