...
Each spawn selects one owner for both signalling and quiescence. Supported Linux ordinary and terminal launches use transient user-systemd scopes, while supported Windows ordinary launches use a helper-owned kill-on-close Job. macOS, older or unavailable user-systemd, and unavailable Windows native support use the existing detached process-group, `taskkill`, or terminal-session observations with one warning. The provider never replays a command through fallback after a native path may have started it. A Linux launcher terminated by an observed OS signal preserves that outcome even before its bootstrap reads the launch request. While a Linux scope is becoming visible, a missing-unit cancellation is retained at its strongest signal and replayed when the manager reports the scope active; if the launcher has already exited while its request remains unread and the manager reports a loaded active scope, the owner requests that exact scope stop and waits for inactive or absent confirmation. A successful scope kill clears a pending signal only when it is at least as strong as that request, and an inactive or failed scope discards it. Recorded bootstrap errors still take precedence; an unsignalled exit with an unread request remains a startup error.
