...
- 被寻址的 subagent 会话继续隐藏。可调模型统一显示 `Low / Medium / High / XHigh / Max / Ultra` 六档；每个视觉档位都会映射到该模型由 Host 公布的最强不超档位，超过模型上限的档位会收敛到真实上限（例如 High-only 模型的 Max／Ultra 都提交 High），并在控件中同时标注视觉档与实际值。只有完全没有正向推理能力的模型才隐藏滑块；若模型公布 Off，Low 端仍可到达 Off。
