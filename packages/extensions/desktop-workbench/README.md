...
- The panel width is limited to 420–1,600 px, starts at 720 px, and persists locally. It remains a real resizable grid column: it may temporarily take the viewport while the left navigation concedes to its compact rail, but it never floats over the conversation or composer. Browser mode serializes exact x/y/width/height updates from the live DOM host into the native `WebContentsView`, including position-only layout changes, so it stays inside the right workbench instead of drifting over blank center content. Terminal polling exists only while Terminal mode is mounted; Browser native resources are destroyed on unmount; no idle animation runs after reasoning text catches up.
