...
- The panel width is limited to 300–720 px, defaults to 360 px, and is persisted locally. The last valid mode is stored separately and restored on the next explicit open without opening the panel for a new Session. The vertical launcher supports Up/Down/Home/End, Enter or Space activation, and Escape to close. Terminal polling exists only while Terminal mode is mounted; Browser native resources are destroyed on unmount; no idle animation runs after reasoning text catches up.
- The Plugins page presents BrowserSkill and Open Design. Open Design status is read from the official `open-design` profile only when this page opens and only a path-free state crosses into the client. BrowserSkill remains dormant until the user explicitly selects Check.
