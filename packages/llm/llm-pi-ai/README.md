...
When neither the profile nor the conversation selects a reasoning effort, a reasoning model defaults to its highest supported level. Explicit selections, including `off`, remain authoritative; a model without reasoning metadata gains no invented levels.
...
An exact `modelId` lookup returns local preset candidates without reading credentials or making an HTTP request. Matching uses the configured route or exact endpoint first, then exact IDs with a compatible protocol. Cross-provider matches take the smallest known capacities and only share reasoning/protocol defaults when those declarations agree. Endpoint-reported values and user edits take precedence; unknown IDs stay editable without fabricated capabilities. Discovery returns detached adapter configuration with the candidate, and never writes settings itself.
