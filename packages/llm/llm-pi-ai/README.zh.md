...
profile 和会话均未指定推理强度时，推理模型默认选择自身支持的最高等级。包括 `off` 在内的显式选择仍然优先；没有推理能力声明的模型不会凭空获得等级。
...
指定精确 `modelId` 的查询直接返回本地预设候选，不读取凭据，也不发送 HTTP 请求。优先匹配已配置路由或完整端点，其次按兼容协议匹配完全相同的 ID。跨提供方匹配采用已知容量的最小值，仅在推理与协议声明一致时补充这些默认配置。端点返回的数值与用户手动修改优先；未知 ID 保留手动编辑，不猜测能力。返回的适配器配置与目录对象分离，发现操作本身不会写入设置。
...
所选目录模型会收到一条系统提示（`GenerateOptions.system`，否则取历史中首条 `system` 消息的文本；首条 system 消息文本为空时不发送系统提示）、其余历史、工具与 pi-ai 通用流式 API 支持的采样字段。每张保留图片前都会有文本，注明其完整附件 id 与实际请求尺寸。当前执行文件系统可以映射附件提供方的宿主对象时，该文本还会携带只读规范化对象路径，并警告规范化或请求投影可能缩放或重新编码上传内容。路由的 `maxRequestImageBytes` 是确定性的图片投影预算：它计算保留图片的 base64 与被省略出现所生成的占位文本，并在转码完成后再次按真实请求图片字节检查。每张卸载图片都会在替换文本中保留自己的身份与当前已解析访问方式。普通用户文本、图片描述、工具、系统提示词与 JSON 框架仍在该图片预算之外，需要单独为请求体预留空间；这个字段不保证完整 HTTP 请求不再触发网关 413。卸载的规范化附件不会读取或变换。提供方原生回放元数据只在适配器针对历史内容校验通过后恢复。
...
- **`maxRequestImageBytes` 是图片投影预算，不是完整 HTTP body 预算**——它计算保留图片的 base64 与生成的省略占位文本；用户文本、图片描述、工具、系统提示词与 JSON 结构在该上限之外，因此路由仍需为请求体预留空间。卸载是确定性请求投影，不会记录为会话事件。
