...
已识别的文档使用本地文档存储：PDF、DOCX、XLSX 及支持的 UTF-8 文本格式会进行有界且不执行内容的文本提取。原始字节与提取文本是不可变的内容寻址对象。请求投影读取经过校验的提取文本；渲染器历史只接收清理后的展示元数据，不含存储标识符、提取文本或宿主路径。
...
| `maxImageBytes` | `50 MiB` | 单张图片接受的最大编码源字节数 |
...
| `maxImageDimension` | `16384` | 源图接受的最大宽度或高度 |
...
准入允许每条消息最多 20 张图片与 200 MiB 源字节；单个源图最多 50 MiB、6400 万像素与单边 16384 像素。这些是分类上限，不是可同时填满的上限：提交还受提示承载约束，后者统计的是 base64 码元而非解码字节。系统应用方向、移除元数据与色彩配置，并把规范化结果限制在 2048×2048 总像素预算、8192 像素长边和 4 MiB 编码字节目标内，因此极端宽高比会保留短边分辨率。已经满足限制的干净、单帧、8-bit sRGB/sRGBA PNG、JPEG 或 WebP 会逐字节直通；GIF、动画、元数据、方向、16-bit PNG 与不兼容色彩空间会触发转换。
