...
文档准入支持 PDF、DOCX、XLSX、UTF-8 文本、Markdown、JSON、CSV、YAML、XML 和常见源代码文件。每条消息最多包含五个文档，源文件总量不超过 50MiB，单个文档不超过 20MiB。每个文档最多保留 96KiB UTF-8 提取文本，每条消息最多 256KiB，截断事实会写入持久引用。PDF.js 在专用 Worker 中只读取分页文本，不渲染，也不执行嵌入动作；Host 施加 30 秒墙钟期限和 128MiB V8 old-generation 上限，丢弃而不记录解析器诊断，并把 Worker 失败映射为闭合附件错误。页数、文本 item 工作量、单 item 字符数与输出字节分别受限。OOXML 展开受条目数、单条目大小、展开总量和路径深度约束；路径穿越、加密容器和宏会被拒绝。DOCX 只提取段落文本；XLSX 只读取已保存的显示值与共享字符串，绝不计算公式；外部关系不会被跟随。每次读取返回数据前，都会同时验证不可变源文件和提取文本的摘要。
...
不发布不变式伴生入口，因为附件边界在 Host 边界由调用方强制。
...
该包通过重启和 fork 后对历史用户图片与文档的持久回放间接影响模型。模型适配器接收确定性的文档提取文本，而不是宿主文件路径。本包通过请求描述符间接影响模型。执行文件系统可以映射宿主对象时，模型会随请求字节看到每张图片的身份、尺寸、媒体类型、只读进程路径、可写副本扩展名与规范化警告。
