...
Recognized documents use the local document store: PDF, DOCX, XLSX and supported UTF-8 text formats receive bounded, non-executing text extraction. Original bytes and extracted text are immutable content-addressed objects. The request projection reads verified extracted text; renderer history receives sanitized display metadata without storage identifiers, extracted text or host paths.
