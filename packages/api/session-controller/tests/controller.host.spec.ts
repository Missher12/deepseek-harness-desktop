...
describe('SessionController facade', () => {
  ...
  it('refuses permanent deletion until the Session is archived', async () => {
    const ctx = new Context()
    await ctx.plugin(SessionStore)
    await ctx.plugin(AgentRegistry)
    const sessionId = SessionId('delete-not-archived')
    const deleteSession = vi.fn(() => Promise.resolve(true))
    const purgeSession = vi.fn(() => Promise.resolve())
    ...
    await expect(controller.delete({ sessionId })).rejects.toMatchObject({
  ...
  it('deletes a cold archived Session and purges its Workspace references', async () => {
    const ctx = new Context()
    await ctx.plugin(SessionStore)
    await ctx.plugin(AgentRegistry)
    const sessionId = SessionId('delete-cold-archived')
    ...
    expect(purgeSession).toHaveBeenCalledWith(sessionId)
