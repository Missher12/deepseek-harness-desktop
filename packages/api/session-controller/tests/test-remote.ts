    ...
    ctx.provide('attachments', {
      ...
      admitPromptContent: async (
        ...
        for (const part of content) {
          if (part.type === 'image' || part.type === 'document') throw new Error('test did not configure attachment persistence')
          admitted.push(part)
        }
        return admitted
      },
  ...
  try {
    controller = new SessionController(
      ctx,
      {
        ...
        ...defaults.noProjectDirectory === undefined ? {} : { noProjectDirectory: defaults.noProjectDirectory },
      },
      {
