...
| `noProjectDirectory` | 未设置 | 未指定 Workspace 或 cwd 的新 Session 的目录父级；相对路径以 Host 用户目录为起点 |

Desktop 设置 `noProjectDirectory: deepseek-temp`，使每个新建的非项目 Session 在 macOS/Linux 使用持久的 `~/deepseek-temp/<sessionId>/` 工作目录，在 Windows 使用用户目录下的对应路径。Session header 记录该 cwd，相对路径的工具输出写入其中。接管已有 Session 时保留所记录的 cwd。显式 Workspace/cwd 与分支会话的目标位置保持原有行为。删除 Session 历史不会删除生成文件。新目录标识使用小写 ASCII 字母、数字、连字符和下划线，并排除 Windows 设备名；符号链接或目录联接目标会失败。该策略选择默认输出位置，不改变文件系统权限。
