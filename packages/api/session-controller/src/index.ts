...
import {
  Remote, RemoteError, TypertRemoteService, remoteErrorOf,
...
export class SessionController extends TypertRemoteService {
  ...
  /**
   * Permanently delete one archived, idle ordinary Session.
   * @param request - archived Session identity to remove.
   * @returns acknowledgement after persistence and Workspace references are purged.
   */
  @Remote('delete')
  async delete(request: SessionDeleteRequest): Promise<SessionDeleteValue> {
    ...
    try {
      const release = await this.agents.releaseForDelete(sessionId)
      ...
      if (release.cold) {
        ...
        if (header !== undefined && hasApiSessionSubagentOwner(this.ctx, { header }, undefined)) {
      ...
      if (release.cold) this.ctx.emit('api-session/removed', sessionId)
      return { deleted: true }
    } catch (error: unknown) {
