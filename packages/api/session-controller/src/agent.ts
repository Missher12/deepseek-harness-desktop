...
export class ApiSessionAgentController {
  ...
  /** Exact top-level Agent capabilities this controller may release for permanent deletion. */
  ...
  constructor(private readonly ctx: Context) {
    ...
    ctx.on('agent/disposed', ({ agent }) => {
      ...
      if (owned?.agent === agent) this.ownedHandles.delete(agent.id)
    })
  }

  /**
   * Retain the exact disposal capability returned by the Agent registry.
   * @param handle - handle created or resumed on behalf of this controller.
   * @returns the published Agent.
   */
  retainAgentHandle(handle: AgentHandle): Agent {
    this.ownedHandles.set(handle.agent.id, handle)
    return handle.agent
  }

  /**
   * Validate and release one live ordinary Agent before durable deletion.
   * Cold sessions require no release and are checked by the controller against persistence.
   * @param sessionId - archived Session identity being permanently deleted.
   * @returns whether no live Agent existed at the deletion boundary.
   */
  async releaseForDelete(sessionId: SessionId): Promise<{ readonly cold: boolean }> {
    if (this.resumes.has(sessionId) || this.creations.has(sessionId)) {
      throw new RemoteError(
        'session/agent-busy',
        `session "${sessionId}" is being activated`,
        { reason: 'session-activation-in-progress' },
      )
    }
    ...
    if (live !== undefined && owned?.agent !== live) {
      throw new RemoteError(
        'session/agent-busy',
    ...
    if (owned === undefined) {
      throw new RemoteError(
        'session/agent-busy',
        `session "${sessionId}" has no owned disposal capability`,
        { reason: 'unowned-session' },
      )
    }
    try {
      ...
      if (this.ctx.agents.get(sessionId) !== owned.agent
        && this.ownedHandles.get(sessionId) === owned) {
        this.ownedHandles.delete(sessionId)
      }
    }
    return { cold: false }
  ...
  private async resumeObserved(
    ...
    return this.retainAgentHandle(await this.ctx.agents.resume({
  ...
  private async createOrAdopt(
    ...
    return this.retainAgentHandle(await this.ctx.agents.create({
      sessionId,
      agentOptions: this.agentOptions(),
      meta: {
        cwd,
