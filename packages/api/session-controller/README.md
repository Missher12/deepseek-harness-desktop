...
| `noProjectDirectory` | unset | Parent for new Sessions without a Workspace or explicit cwd; relative paths start at the Host home |

Desktop sets `noProjectDirectory: deepseek-temp`, giving each new no-project Session a persistent `~/deepseek-temp/<sessionId>/` working directory on macOS/Linux and the equivalent under the Windows user profile. The Session header records that cwd, and relative tool output uses it. Adopting an existing Session preserves its recorded cwd. Explicit Workspace/cwd and fork destinations retain their existing behavior. Deleting Session history does not delete generated files. New directory identities use lowercase ASCII letters, digits, hyphens and underscores, excluding Windows device names; symbolic-link or junction targets fail. This policy selects a default output location and does not change filesystem permissions.
