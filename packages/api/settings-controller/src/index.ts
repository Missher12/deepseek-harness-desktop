 ...
 * seams. Two namespaces: `settings`, the redacted `ctx.settings` operations
 * plus the fixed global-personalization document, owned by the class below;
 * and `credentials`, mounted from here as its own plugin.
 *
...
export interface SettingsControllerInternals {
  ...
  /** Fixed Host-owned target override for direct storage tests. */
...
/**
 * Host service backing the generated `ctx.remote.settings` namespace. Provider
 * reads use `redactSecrets: true`, so a `role('secret')` field cannot ride a
 * response; personalization projects only its browser-safe managed block and
 * never the Host path. Writes expose the settings service's merge, replacement,
 * and path-addressed operations, and classify every refusal as
 * `settings/conflict` or `settings/rejected` with the owner's message.
 */
export class SettingsController extends TypertRemoteService {
  ...
  /**
   * Read the Desktop-managed block from the fixed global AGENTS.md.
   * @returns browser-safe content and ownership facts without exposing the path.
   * @throws RemoteError when the Host cannot safely inspect the document.
   */
  @Remote
  async personalizationRead(): Promise<PersonalizationDocumentView> {
    try {
      ...
      throw personalizationRejected(error)
    }
  }

  /**
   * Replace only the Desktop-managed global personalization block.
   * @param input bounded content and the optimistic revision read by the caller.
   * @returns the authoritative browser-safe document state after writing.
   * @throws RemoteError when validation, ownership, revision, or storage refuses the write.
   */
  @Remote
...
function personalizationRejected(error: unknown): RemoteError {
