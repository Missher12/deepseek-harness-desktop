...
Host adapters can depend on stable, swappable services without importing Electron or native helper code. Provider implementations have reusable owner/freshness and immutable-result checks, while later bridge/helper layers remain responsible for actually authorizing, minting, caching, revoking, and cleaning leases, plus permissions, quotas, and native execution. Conservative classification may refuse some safe-looking controls until an adapter can identify them authoritatively; that is the deliberate cost of failing closed.
