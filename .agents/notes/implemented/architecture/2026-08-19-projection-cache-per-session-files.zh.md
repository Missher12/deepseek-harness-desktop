...
缓存先对可变投影状态拍快照并预留该会话的写入位置，再启动日志持久性屏障。各次日志 flush 可以独立完成，但后续检查点必须等待前一个请求结算，才能进入存储域的写入链。一次失败不会破坏后续请求。冷读取刷新使用同一排序，释放时停止接收新请求，排空已接收的检查点后才关闭存储域。文件操作和持久发布仍归存储域所有，缓存只排列发布之前的工作。真实 Cordis/JSON 屏障测试覆盖 flush 逆序完成、后续状态变更、独立会话、flush 失败恢复和释放。

## Consequences

- 每会话写入隔离：每次节流写入只替换该会话的小文档，消除全局写放大。缓存在等待日志持久化期间保持请求顺序，存储域串行执行持久发布；释放时排空这两个阶段。
