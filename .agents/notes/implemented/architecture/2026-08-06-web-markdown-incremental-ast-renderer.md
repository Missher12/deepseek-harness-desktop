...
- **Incremental parsing** ([incremental.ts](../../../../packages/client/ui-primitives/src/markdown/incremental.ts)): the parser retains the last two top-level blocks and freezes earlier blocks at grammar-produced source offsets. Unclosed top-level fences reuse completed content lines. Tight paragraph lists use [list-frontier.ts](../../../../packages/client/ui-primitives/src/markdown/list-frontier.ts) to retain completed items and parse only the last item and appended content. Fresh list nodes are rebased into the original source coordinates without mutating earlier snapshots or changing ordered-list starts. Loose/nested lists, references, HTML, and following blocks return to the complete tail grammar in the same update. Other growing blocks retain their full-tail cost. Non-append input resets every frontier under a bumped generation.
