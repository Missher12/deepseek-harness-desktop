...
- **增量解析**（[incremental.ts](../../../../packages/client/ui-primitives/src/markdown/incremental.ts)）：解析器保留最后两个顶层块，并在语法生成的源码偏移处冻结更早的块。未闭合的顶层 fence 复用已完成的内容行。紧凑段落列表通过 [list-frontier.ts](../../../../packages/client/ui-primitives/src/markdown/list-frontier.ts) 保留已完成项，只解析最后一项与追加内容。新列表节点会转换到原始源码坐标，不修改早先的快照或有序列表起始编号。松散／嵌套列表、引用、HTML 和后续块在同一次更新中回到完整尾部语法。其他增长中的块仍承担完整尾部解析成本。非追加输入会增加 generation 并重置全部前沿。
...
DOM 由 `tests/fixtures/markdown-dom` 钉死:fixture 录制自替换前的 react-markdown 实现,新渲染器必须在空白规整序列化器下复现。fixture 差异即用户可见的 markdown 样式变更,必须按此评审,绝不能为重构而重录。`tests/markdown-incremental.client.spec.tsx` 承载等价性性质——以 1/3/7/16 字节分块,在每个追加前缀处,常驻组件的 DOM 都等于全新挂载——外加冻结边界的 DOM 节点同一性与重置行为。
