...
The cache snapshots mutable projection state and reserves a per-session write position before starting the log durability barrier. Log flushes may finish independently, but a later checkpoint waits for its predecessor to settle before entering the storage domain's write chain. Failed writes do not poison later requests. Cold-read refreshes use the same ordering, and disposal stops admission and drains accepted checkpoints before closing the domain. The domain still owns every file operation and durable publication; the cache orders the work that precedes that publication. Real Cordis/JSON barrier tests cover reversed flush completion, later mutation, independent sessions, failed-flush recovery and disposal.

## Consequences

- Per-session write isolation: each throttled write replaces only that session's small document, removing the global write amplification. The cache preserves request order across log durability waits, and the domain serializes durable publication; disposal drains both stages.
