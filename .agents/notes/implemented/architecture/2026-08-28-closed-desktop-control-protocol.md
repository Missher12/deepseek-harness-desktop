...
The binary envelope separates bounded UTF-8 JSON from raw PNG bytes. Encoding first inspects a plain descriptor-only value tree, rejects custom serialization hooks and non-data state, then strictly decodes and validates the exact emitted frame. A transport can inject a trusted JSON-message validator, which sees only detached JSON, cannot return data, and runs before screenshot correlation can become pending; this lets each bridge direction reject a valid-but-wrong-direction message immediately. Screenshot metadata then requires the immediately following PNG and binds its UUID, byte length, SHA-256, width, and height. Decoder output is detached and deeply frozen; image bytes have one immutable owner whose reader returns copies. A validator rejection or returned value, malformed value, or frame sequence closes only the decoder used for Desktop control. Bridge deadlines are accepted only in the interval `(nowUnixMs, nowUnixMs + 30_000]` using the caller's single time sample.

The Desktop-owned Node IPC peers now apply those direction validators before correlation: Harness sends only the 27 bridge requests, cancellation, and session revocation; Electron sends only matching responses with any adjacent PNG, lease revocation, and parent shutdown. Each IPC message contains one copied unprefixed frame. The helper's four-byte stream prefix remains exclusive to the later native-helper transport.
...
Every new operation or result requires one reviewed manifest and TypeScript update plus fixture changes, and the later native implementation must prove byte parity. Raw acquire/release frames are checked in beside the status and screenshot fixtures. The package defines lease lifecycle messages but deliberately does not mint leases or own authorization, request ledgers, process generations, or image normalization; the Desktop Host and Electron bridge own those stateful transport concerns while later authority consumes the validated values. Protocol v1 has no compatibility negotiation during the repository's pre-release phase.
