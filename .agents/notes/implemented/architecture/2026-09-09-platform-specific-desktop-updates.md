...
Each owned Windows PowerShell process disables automatic module discovery only within itself. Bootstrap, worker and exact-worker cleanup explicitly import the Utility manifest under the fixed executable's `$PSHOME` before JSON; bootstrap and worker import its Management manifest before process creation. No user module path, execution policy or machine preference is changed. Fixed bootstrap codes bracket both imports and JSON completion. This removes discovery-dependent command resolution; whether it resolves the observed preflight timeout requires native evidence. When the command budget requires it, optional phase records and fixed-script indentation are omitted, never imports, direct checkpoints or handoff checks.
...
Windows bootstrap and worker phase records are best-effort private observations, never readiness or installation authority. Flushed bootstrap stderr checkpoints use only .NET calls around configuration parsing, identity reads and native worker creation. Native preflight output contains ordered fixed codes, identity-match booleans and bounded byte counts; process identities, paths and raw stderr remain private. The preflight retains its first failure independently of cleanup failures, cancels before stopping known workers and retained children, and preserves private files when cleanup is unconfirmed. Missing diagnostic records do not reject an otherwise verified handoff; observation statements are omitted when needed to preserve the fixed command-line size limit. The 20-second bootstrap readiness limit, 30-second application watchdog and 120-second parent-exit limit remain independent.
