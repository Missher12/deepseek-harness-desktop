...
**Models 界面通过一个保守的三态控件编辑 `input`。** 在产品自身的自定义提供方流程使该缺口变成用户可见的能力误报后，[自定义模型输入能力界面](../bug-fix/2026-08-27-custom-model-input-capability-surface.zh.md)取代了此前“只在 settings 文档配置”的决定。自动仍省略该字段并遵循本记录的解析链；另外两个显式选择分别写入仅文本或文本加图片声明。精选表单不编辑的字段仍会在行 patch 和重新发现中保留。
...
自定义提供方下的视觉模型可以从 Models 界面声明，也可以在 settings 文档里写一行 `input: [text, image]`——或者当该路由列出的模型全都接受图片时，在路由上写一行。三个准入点随即接纳它上面的图片，`read_image` 也可用。什么都不写的部署保持原有行为完全不变，因此没有任何既有路由改变它报出的内容。
