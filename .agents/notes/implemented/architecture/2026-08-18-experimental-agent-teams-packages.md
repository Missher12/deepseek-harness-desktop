...
Profile startup resolves selected bundles before healing module fallbacks. The shared fallback retains the dsh installation's carrier-specific entries: symlinks under plain Node and ESM proxies in a packaged executable. Missing packages from selected bundle closures stay under the current profile's own `node_modules`, while pnpm-managed profile entries remain authoritative. Physical targets use links; a target inside pkg's `/snapshot` or Electron's `app.asar` uses a managed profile-local ESM proxy because the operating system cannot traverse that archive boundary. Closure discovery starts from each explicit external bundle's real package directory and traverses every listed root even when an earlier dependency has the same package name. It excludes dsh-owned profile projections from later discovery, so a projected dependency cannot feed back into its own closure. Link and proxy ownership remain explicit and removable. A private profile layer can therefore carry experimental plugin rows without adding those plugins to a release app, requiring profile users to install transitive packages directly, weakening packaged-runtime module identity, or changing another profile's resolution.
