...
启动冲突检测使用操作系统安装的 `lsof` 检查另一个 Harness Web 进程：Linux 为 `/usr/bin/lsof`，macOS 为 `/usr/sbin/lsof`。只有同一个 Harness 数据目录下的文件才能确认冲突。可执行文件缺失和检查被拒绝仍按失败处理，不能据此认定没有其他写入进程。
...
Debian 安装管理桌面入口、图标、依赖和 AppArmor 策略。Ubuntu 24.04 的 AppImage 用户需先运行匹配的准确路径辅助脚本；路径变化后要更新策略。

Ubuntu 的[系统更新](../../../../packages/client/ui-settings-system-update/README.zh.md)范围涵盖版本检查、匹配已安装 `.deb` 或 AppImage 格式的已验证下载，以及打开安装包目录。用户完成实际安装；Desktop 不自动提权、执行下载的安装包或覆盖正在运行的 AppImage。

ARM64 和 Wayland 仍未验收。安装包构建成功不能单独作为发布证据；发布需要在 Ubuntu 22.04 和 24.04 上，针对同一最终共享源码版本的两种包格式完成原生生命周期与系统更新检查。这一范围说明不代表实际未来公开版本升级已通过验收。

## 测试

本地测试覆盖暂存、原生窗口配置、宿主专属的打开文件查询命令、不同 Harness 数据目录、检查失败、Linux 启动包装器及内核观测的接受和拒绝，包括 Chromium 用空格连接参数的进程标题。原生矩阵保留安装包字节摘要和失败证据。平台验收状态记录在发布交接中，不在这份实现说明中维护。
