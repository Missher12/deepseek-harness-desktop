...
当某块是流式尾部时，`useRevealedText` 逐帧绘制逐渐增长的前缀。完整原文从第一帧起就挂在 `data-reasoning-full` 上，因此 DOM 从不把残缺的思考当作该块的全部内容。帧通过 `requestAnimationFrame` 调度，并按经过的时间推进，因此在 60、120、240 Hz 下显示速率一致；该 hook 不数帧，也不假设固定间隔。

## Reveal budget

`REVEAL_CLUSTERS_PER_SECOND` 为 240。`CATCH_UP_CLUSTERS` 与之相等：显示最多欠模型约一秒的打字量，之后便欠读者整个分块——超过预算的积压在一帧内整体绘制，而等待超过 `BACKLOG_CHASE_MS`（300 毫秒）的积压无论大小都会追齐。扩展上一段文本的分块保留已绘制计数，因此流式追加会继续播放；替换文本的分块则重新开始，因为该计数描述的是它所测量文本的前缀。字素边界来自 `Intl.Segmenter`，因此家庭 emoji、组合字符或 CRLF 都不会被截断在半个字素上。
