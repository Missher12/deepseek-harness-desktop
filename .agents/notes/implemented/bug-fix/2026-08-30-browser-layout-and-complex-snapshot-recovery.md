...
Because an Electron `WebContentsView` always paints above renderer DOM, Browser mode also publishes whether its host is actually exposed. A modal dialog, a hidden document, or another renderer surface crossing any sampled part of the host hides both human and Agent native views without destroying their page. Electron main retains that visibility decision across later layout updates and newly registered mounts, so resize, transfer, or renderer startup cannot reveal a view behind Settings. Closing the occluding surface restores only the still-owned mount in the same rectangle.
...
Renderer dialogs can cover the whole application without native browser pixels punching through them. Closing a dialog restores the same browser page and ownership state rather than creating a new tab or resetting navigation. Switching between Browser, Terminal, Files, and Review now suspends only native pixels; returning to Browser reuses the exact human view and URL. Lifecycle failure, close-to-tray, and application shutdown still perform destructive cleanup.
