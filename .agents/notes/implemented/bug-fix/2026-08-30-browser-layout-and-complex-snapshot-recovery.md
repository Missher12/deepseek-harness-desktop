...
The utility stays a real fourth grid column. It defaults to 720 pixels, grows to 1,600 pixels, shrinks first, and then temporarily reduces the rendered left navigation to its 56-pixel rail without changing the user's saved width. A truly narrow viewport focuses the utility in the remaining grid; it never overlays the conversation. Composer height bounds the prompt rail, and the status line uses a two-line bounded flow layout.
...
The workbench can occupy most of a desktop window while the saved navigation width remains intact. On very narrow windows the conversation is intentionally hidden until space returns, rather than being covered by another pane. Windows users see bounded installation completion stages; extraction internals remain controlled by electron-builder.
