...
Because an Electron `WebContentsView` always paints above renderer DOM, Browser mode also publishes whether its host is actually exposed. A modal dialog, tooltip, hidden document, or another renderer surface intersecting the host hides both human and Agent native views without destroying their page. Visibility and hide operations share one serialized generation queue across remounts, so a stale cleanup cannot hide a newer Browser instance. A visible-window resynchronization reapplies even unchanged bounds after minimize or another non-destructive window visibility restoration, while address hydration follows the preserved native page without overwriting edits typed during an asynchronous snapshot or later state push. Electron main validates exact IPC arity and retains the exposure decision across later layout updates and newly registered mounts, so resize, transfer, or renderer startup cannot reveal a view behind Settings. Closing the occluding surface restores only the still-owned mount in the same rectangle. Closing the native window to its keep-running state remains lifecycle cleanup and intentionally starts a fresh human browser page when the window is shown again.

The browser walker accepts up to 10,000 raw nodes and 300 actionable refs while preserving the existing 64 KiB encoded result boundary. Raw-node, CDP-call, actionable, semantic-text, and encoded-result limits return a deterministic partial snapshot. Candidate ordering retains focused, editable, and button-like controls before link noise. The model prompt treats partial output as usable and forbids unchanged snapshot retry loops. Debugger startup gets a bounded twenty-second, two-attempt window. Loading and navigation waits return the current revision at their ten-second bound. Click and scroll operations show a main-owned blue CDP Overlay marker without page code or selector injection.
...
Renderer dialogs and intersecting tooltips can cover the whole application without native browser pixels punching through them. Closing them restores the same browser page and ownership state rather than creating a new tab or resetting navigation. Switching between Browser, Terminal, Files, and Review now suspends only native pixels; returning to Browser reuses the exact human view, URL, and address display. The utility layout has one persisted owner, its separator exposes keyboard resizing, and the header reflects that same state. Long run statistics expand inline on click or focus instead of opening a floating tooltip over the composer. Lifecycle failure, close-to-tray, and application shutdown still perform destructive cleanup.
