...
While a block is the streaming tail, `useRevealedText` paints a progressively growing, grapheme-aligned prefix. The visible body is the accessibility and selection channel; `data-reasoning-full` records the received text for diagnostics and tests only, and does not provide a complete-text reading or copy interaction. Terminal, paused, and reduced-motion states paint the complete text. Frames are scheduled with `requestAnimationFrame` and stepped by elapsed time, so the revealed rate is derived from elapsed time rather than a frame count or fixed interval.

## Reveal budget

`REVEAL_CLUSTERS_PER_SECOND` is 240. `CATCH_UP_CLUSTERS` equals it, so the reveal owes the model at most about one second of typing before it owes the reader the whole chunk: a backlog larger than the budget is painted in a single frame, and a backlog pending past `BACKLOG_CHASE_MS` (300ms) is caught up regardless of size. A chunk that extends the previous text keeps the painted count, so a streaming append continues the reveal; a chunk that replaces it restarts, because the count described a prefix of the text it was measured against. `Draft` segments with `Intl.Segmenter` and retains the mutable tail context needed for a newly completed surrogate pair, regional-indicator pair, or emoji join sequence to merge with the preceding cluster. The formal stream tests compare every UTF-16 split and read the rendered body text. No current Draft performance benchmark establishes a 120 or 240 Hz browser-frame guarantee.
