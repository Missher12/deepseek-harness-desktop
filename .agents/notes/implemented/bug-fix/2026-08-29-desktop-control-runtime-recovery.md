...
Browser address fields exposed two macOS interoperability gaps: some applications identify the focused child through the application-level `AXFocusedUIElement` while omitting the child's `AXFocused` boolean, and dynamic accessibility siblings can reorder between snapshot and input. Requiring only the child boolean rejected text entry after a successful semantic click; deriving refs from traversal positions then made the correct address field appear stale. Multi-event Unicode delivery could also change that field's own accessible name or geometry after the first event and incorrectly reject its required release event.
...
macOS projection marks an editable or sensitive element focused when either its own `AXFocused` fact is true or its AX identity equals the application's `AXFocusedUIElement`. Refs hash that process-local native AX identity with the target and snapshot revision instead of hashing a traversal index; duplicate identities fail closed. Input still requires that exact ref, an editable non-sensitive classification, current process/window identity, permissions, lease, capability, quota, deadline, and cancellation checks before every native event. Only Unicode delivery may accept name or geometry changes caused by its own preceding event, and only while native identity, role, editability, sensitivity, and focus remain valid.
...
Browser text entry works with accessibility implementations that publish focus only at the application level or reorder dynamic siblings, and a field may update during the helper's own bounded text delivery without losing its release event. Secure, ambiguous, identity-changed, stale, or unfocused fields remain protected. Agent Browser navigation tolerates ordinary Windows resolver latency without turning its authenticated proxy into an unbounded or unauthenticated service.
