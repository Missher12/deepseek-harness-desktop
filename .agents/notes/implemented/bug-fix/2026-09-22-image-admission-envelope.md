...
Request assembly chooses the removal count from the payload the request will actually carry. `fitRequestImageBudget` walks the removal counts, prices each outcome as the kept images' base64 plus every placeholder's text, and takes the fewest that fits. When no count fits — the placeholders alone exceed the bound — it falls back to the byte-only projection, so the closest reading is still sent and every omitted image stays identifiable by its placeholder rather than being dropped silently.
...
A 25.8 MiB source that the previous byte limit refused is now admitted and normalized; a 16384px long edge passes where 8192px was the ceiling. Request assembly now removes enough images that the payload it sends fits, which costs a few more omissions than the byte-only reading chose and buys the request the bound was set to produce. When even replacing every image cannot fit, the closest reading is sent and each omitted image keeps its placeholder, so nothing is dropped silently.
