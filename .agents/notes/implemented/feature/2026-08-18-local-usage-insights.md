...
Each fold owns one validated time-zone formatter. Calendar projection runs only for events contributing daily counts or usage samples, so streamed text and reasoning chunks have no formatter-allocation cost. Midnight and daylight-saving transitions retain local-calendar semantics.
