...
客户端解析**不在项目中**目标时，排除 cwd 等于已注册 Workspace 路径的空白 Session。会话创建可能先于 Workspace 关联更新变得可见，因此需要同时检查两类信息，避免在此间隔误用项目会话。安装包验收还会复制一份 V2 日志，不通过测试侧持久化提供方打开它；先等待运行中的产品生成 V3，再检查原始字节，并对独立目录、重新打开、显式项目和迁移会话执行五次真实 write 工具调用。

可移植原生验收记录仅包含 Session 目录叶名称、保留结果、迁移版本和哈希。三端打包流程使用同一个校验器检查后再保留这些记录，不包含 Host 绝对路径。
