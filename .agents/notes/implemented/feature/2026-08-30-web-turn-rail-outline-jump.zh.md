...
历史窗口头部的半轮可能在首条实际消息之前包含一个合成的 Turn 过程控制节点。这个控制节点不作为导航目标：历史尚未完整加载时，它的 seat 可能被隐藏；选中它的 key 会导致点击后不滚动便返回。已加载锚点会跳过该控制节点，存在用户消息时仍优先定位用户消息。组件回归验证旧行为确实不滚动，完整 Desktop 网页和两端安装包 smoke 都通过真实指针点击导航栏，并断言所选 transcript 行到达阅读线。
