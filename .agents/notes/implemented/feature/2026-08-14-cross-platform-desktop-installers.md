...
The dedicated `Windows Desktop Setup` workflow and the repository's native Windows pull-request job run on GitHub's standard public `windows-2025` runner. The dedicated workflow is the bounded release path: immutable install, complete product build, Setup packaging, isolated install, both-shortcut verification, packaged launch with temporary Harness and Electron data, settings interaction, native close, listener and process-tree cleanup, uninstall, data-marker preservation, SHA-256, and artifact upload. The broader native job retains the complete Windows inventory as additional evidence; the self-hosted master standby runs the same packaging lifecycle when such a runner is configured.
