...
- Electron Builder emits one unsigned Windows x64 NSIS Setup in assisted, current-user, non-elevating mode. The installer restores NSIS status and detail output at the running-application check while retaining the upstream check and native extraction progress. Interactive install creates desktop and Start menu shortcuts and offers to launch the application; uninstall preserves Harness and Electron data.
...
Installer acceptance observes the real progress-bar range and position, populated detail rows and non-empty status before completion. Position changes must occur within a single native range while details remain available. A fixed public baseline verifies the empty-details regression; the candidate must pass the populated-details condition. Both checks uninstall normally before evaluating their observations. Evidence retains bounded counts, durations and redacted images.

Windows Search exposes the query field consistently while the rendered result region can omit accessibility descendants. Search acceptance therefore verifies the complete foreground query, sends Enter, and proves that the exact installed executable creates a new process with an owned visible window and a current startup marker. This launch follows the 100-percent tray Quit and process-tree shutdown. It uses fresh default profiles on the disposable runner because Search launches through Explorer's environment. Its owned-process cleanup is recorded separately from the tray lifecycle evidence.
