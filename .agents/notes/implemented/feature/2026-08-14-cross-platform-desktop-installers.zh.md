...
仓库现有的原生 Windows PR 任务会在完整 Windows 检查之后构建 Setup，把它安装到隔离目录，验证两种快捷方式，使用临时 Harness 和 Electron 数据启动已安装程序，打开设置，关闭原生窗口，证明监听端口与应用拥有的进程树已经消失，然后卸载、确认两份数据标记仍然保留、记录 SHA-256，并上传程序和校验文件。自托管 master 备用任务运行相同的构建与生命周期测试。
