...
The rail renders at most 120 marks while keeping the first and last prompt reachable. Below the supported content width, the desktop ruler becomes a compact localized Prompt Navigation trigger with the current and total counts, rather than overlapping the composer or message column. Its lightweight dialog list exposes the same exact anchors, activates the selected sequence once, closes after selection, and restores trigger focus; Escape does the same. A true anchor-index replacement clears stale tooltip and roving-focus state and closes any open compact list; a pre-commit snapshot restores focus only when the rail owned it at replacement, never from the composer or transcript. Dropping below two anchors clears the compact/open, tooltip, roving, and pending-handoff state, so a later reappearance starts closed. Responsive handoff snapshots rail ownership synchronously in the media-query change callback before React commits the new mode, then moves focus between the compact trigger and the effective desktop mark only for that owned focus. Live user messages append anchors locally, while a resync replaces the index from the new immutable tail cut.
...
Host history tests cover tail-only index delivery, normalized previews, long history, and immutable cuts. Session runtime tests cover exact target loading, request serialization, live-anchor updates, and resync. Conversation and real Chromium tests cover left placement, long and short marks, bounded first/last reachability, localized hover and focus tooltips, roving Arrow/Home/End focus without navigation, exact-once Enter/Space activation with no native mark click, reduced-motion tick transitions and immediate scrolling, compact selection, replacement without composer-focus theft, reset below two anchors, breakpoint focus handoff, exact forced-colors system paint, and unavailable-target status. Replayed light and dark browser snapshots cover the approved desktop ruler with repeated pixel calibration plus independent four-tick raster checks, including endpoint-removal mutation probes.
