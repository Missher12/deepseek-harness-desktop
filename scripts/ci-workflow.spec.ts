...
describe('CI workflow', () => {
  ...
  it('validates Windows system entries and real lease exclusion without changing the packaged candidate', () => {
    const windows = workflowJob(loadWorkflow('.github/workflows/windows-desktop.yml'), 'build-install-smoke')
    if (!Array.isArray(windows.steps)) throw new TypeError('Windows Desktop workflow must define steps')
    const steps = windows.steps.filter(isRecord)
    const name = 'Verify Windows system paths without changing packaged bytes'
    expect(steps.filter(step => step.name === name)).toHaveLength(1)
    const validationIndex = steps.findIndex(step => step.name === name)
    expect(validationIndex).toBeGreaterThan(steps.findIndex(step => step.name === 'Build the assisted Windows Setup'))
    expect(validationIndex).toBeLessThan(steps.findIndex(step => step.name === 'Measure the pinned public 0.5.3 baseline on this runner'))
    const validation = steps[validationIndex]
    expect(validation).toMatchObject({
      shell: 'pwsh',
      env: { ARTIFACT: '${{ steps.desktop.outputs.artifact }}', NODE_OPTIONS: '', NODE_PATH: '' },
    })
    if (typeof validation?.run !== 'string') throw new TypeError('System-path validation must define a command')
    ...
    for (const required of [
      ...
      'Provider selected direct-spawn fallback, not Windows Job runner',
      "for (const mode of ['normal', 'cancel'])", 'gone(identity.pid) && gone(identity.parent)',
      'report.primaryError ??= describeFailure(error)', 'report.cleanupError ??= describeFailure(error)',
      "join(jobDirectory, 'process-result.json')", 'Embedded Job report unavailable; original process outcome preserved',
      'if (cleanupConfirmed && accepted) await rm(jobDirectory',
    ...
    expect(run.indexOf('const before = await fingerprint()')).toBeLessThan(run.indexOf('await verifyFlockEntry('))
    expect(run.lastIndexOf('finally {')).toBeLessThan(run.indexOf('assert.deepEqual(await fingerprint(), before'))
    expect(run).not.toMatch(/fs-ext|node-gyp|(?:pnpm|npm) (?:install|rebuild)|electron-builder install-app-deps/u)
  })
