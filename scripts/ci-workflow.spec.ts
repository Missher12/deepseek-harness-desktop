...
describe('CI workflow', () => {
  ...
  it('keeps a required Wine Windows job, a non-blocking native Windows job with failover, and a master-only standby', () => {
    ...
    expect((windowsNative.steps as unknown[]).some(step => (
      ...
      && step.with.path.includes('apps/desktop/release/DeepSeek-Harness-Setup-0.1.3-win-x64.exe\n')
      && step.with.path.includes('apps/desktop/release/DeepSeek-Harness-Setup-0.1.3-win-x64.exe.sha256')
    ))).toBe(true)
...
describe('Windows desktop Setup workflow', () => {
  it('maps the checkout to a drive root so native MSVC rebuilds stay below MAX_PATH', () => {
    ...
    expect(smoke.run).toContain('./scripts/windows-desktop-setup-smoke.ps1 -SetupPath apps/desktop/release/DeepSeek-Harness-Setup-0.1.3-win-x64.exe')
    expect(smoke.run).not.toContain('Set-Location S:\\')
    expect(checksum.run).not.toContain('Set-Location S:\\')
    expect(upload.with.path).toContain('s/apps/desktop/release/DeepSeek-Harness-Setup-0.1.3-win-x64.exe')
  })
})
