...
describe('browser dependency discovery', () => {
  ...
  it('honors the client asset loader while recording its runtime library', async () => {
    const root = fixture()
    library(root, 'browser-lib')
    dynamicPlugin(root, 'import sprite from "./sprite.png"; import { value } from "browser-lib"; export const output = [sprite, value]')
    const config = join(root, 'packages/client/dynamic/tsdown.config.ts')
    writeFileSync(config, readFileSync(config, 'utf8').replace('  inputOptions:', '  loader: { ".png": "dataurl" },\n  inputOptions:'))
    write(root, 'packages/client/dynamic/src/client/sprite.png', 'fixture asset bytes')
    expect(await browserBundledExternals(root)).toEqual(new Set(['browser-lib']))
    expect(existsSync(join(root, 'packages/client/dynamic/lib'))).toBe(false)
  })
