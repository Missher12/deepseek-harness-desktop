...
Desktop 0.3.7 把独立版本的 `dsh-missher-memory@0.1.2` 作为超长项目记忆
兜底内置。用户明确绑定的新项目默认开启候选收集和有界召回，但收集内容只会进入
审核箱，未经批准绝不会进入可搜索记忆。外部来源数据库始终只读，项目彼此隔离，
仅打开设置页不会创建状态数据库。它会在插件市场中显示为“由 Desktop 管理”的
内置插件：市场不能更新、停用、分组或卸载这个兜底版本；以后用户在 profile 中
自行安装同名新版时，profile 版本仍可优先加载。
...
这个命令必须在原生 Windows x64 上运行。Setup 名称由 `apps/desktop/package.json` 派生；0.3.7 会输出 `apps/desktop/release/DeepSeek-Harness-Setup-0.3.7-win-x64.exe`。Windows CI 使用独立的短暂存目录，避免原生 MSVC 重编译触发旧式路径长度限制；所有发布产物都写入 `apps/desktop/release`。
...
./scripts/windows-desktop-installer-ui-smoke.ps1 `
  -SetupPath apps/desktop/release/DeepSeek-Harness-Setup-0.3.7-win-x64.exe
