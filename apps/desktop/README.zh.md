...
Desktop 0.4.6 保留 Codex 风格的发言刻度，并提供两种可见控制能力。
...
0.4.6 的浏览器流程会等待新挂载的渲染进程，在有界语义遍历前启用 Accessibility，
对临时无效的根节点进行重试，并正确处理已经空闲的页面和初始 `about:blank` 历史项，
不再产生错误超时或策略拒绝。Browser Stop 成功后会清除本轮 shell 防绕过状态；失败后
仍不能改走 shell 脚本或直接连接远程调试端口。macOS 原生成品和已安装的 Windows
成品验收都会要求从真实本地语义树读到测试按钮。

会话侧栏和工具工作台可以在实用的大范围内调整宽度，并会保存上次宽度。浏览器模式会
同步原生视图的完整矩形（包括仅位置变化），因此受控页面始终留在可见右侧面板内，
不会漂移到工作区空白位置上方。
...
这个命令必须在原生 Windows x64 上运行。Setup 名称由 `apps/desktop/package.json` 派生；0.4.6 会输出 `apps/desktop/release/DeepSeek-Harness-Setup-0.4.6-win-x64.exe`。Windows CI 使用独立的短暂存目录，避免原生 MSVC 重编译触发旧式路径长度限制；所有发布产物都写入 `apps/desktop/release`。
