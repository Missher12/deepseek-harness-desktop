...
export type DesktopControlAvailability = 'available' | 'unavailable' | 'unknown'
...
function isCapabilityState(value: unknown): value is DesktopControlCapabilityState {
  return plainRecord(value) && exactKeys(value, ['availability', 'enabled'])
    && (['available', 'unavailable', 'unknown'] as readonly unknown[]).includes(value.availability)
