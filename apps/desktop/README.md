...
Desktop 0.4.8 keeps the Codex-style prompt ruler and provides two visible control
...
through bounded accessibility, capture, pointer, and keyboard actions. Electron
main owns the per-task native approval, one process-wide lease, target identity,
...
The 0.4.8 browser path gives the newly mounted renderer a bounded twenty-second
startup window, enables Accessibility before its bounded semantic walk, and
returns prioritized partial refs instead of failing complex pages at a raw-tree
limit. Already-idle and bounded condition waits return the current state; the
initial `about:blank` history entry is not misclassified as protected. A visible
main-owned marker shows click and scroll targets. Browser Stop or a successful
official operation clears recoverable turn-scoped shell guards; failures still
cannot fall back to shell scripts or a direct remote-debugging connection.
Native macOS and installed Windows acceptance both require a real local
semantic button.

The session sidebar and utility workbench are independently resizable and
persist their last widths. The workbench starts at 720px, grows to 1,600px, and
remains a real grid column; under pressure the rendered sidebar concedes to its
compact rail instead of letting the workbench cover the conversation or
composer. Browser-mode geometry synchronizes the complete native view rectangle,
including position-only changes, so the controlled page remains inside the
visible right panel instead of drifting over an empty workspace area.
The exact transferred view now keeps synchronizing while the Agent owns it and
reflows desktop-oriented pages to the live panel width. Optional PNG capture
differences on Retina renderers degrade to the already-verified semantic refs
instead of turning the whole snapshot into `INTERNAL`; native logs record only
a closed failure phase and error code. Prompt-rail height follows the measured
conversation viewport and composer so its tooltip cannot overlap the input.
