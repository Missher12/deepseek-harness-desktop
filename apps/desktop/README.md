...
Desktop 0.4.6 keeps the Codex-style prompt ruler and provides two visible control
...
The 0.4.6 browser path waits for the newly mounted renderer, enables
Accessibility before its bounded semantic walk, retries a temporarily invalid
root, and handles already-idle pages and the initial `about:blank` history entry
without false timeouts or policy denials. A successful Browser Stop clears the
turn-scoped shell fallback guard; failures still cannot fall back to shell
scripts or a direct remote-debugging connection. Native macOS and installed
Windows package acceptance both require a real local semantic button.

The session sidebar and utility workbench are resizable across useful wide
ranges and persist their last widths. Browser-mode geometry synchronizes the
complete native view rectangle, including position-only changes, so the
controlled page remains inside the visible right panel instead of drifting
over an empty workspace area.
...
./scripts/windows-desktop-installer-ui-smoke.ps1 `
  -SetupPath apps/desktop/release/DeepSeek-Harness-Setup-0.4.6-win-x64.exe
