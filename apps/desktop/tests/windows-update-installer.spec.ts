...
describe('Windows update bootstrap and independent worker', () => {
  ...
  it('uses a short system bootstrap with an owned worker handle and visible Setup only after verified parent exit', () => {
    ...
    const entry = script.indexOf("Trace 'E'")
    expect(entry).toBeGreaterThan(0)
    expect(entry).toBeLessThan(script.indexOf('$workerScript ='))
    expect(entry).toBeLessThan(script.indexOf('$config ='))
    expect(script.slice(0, entry)).toContain('[Console]::Error.Flush()')
    expect(script.slice(0, entry)).not.toMatch(/ConvertFrom-Json|ConvertTo-Json|MainModule/u)
