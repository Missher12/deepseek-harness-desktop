...
async function exerciseWindows150PercentSurface(
  ...
  try {
    ...
    const promptRail = page.getByRole('navigation', { name: /^(?:Previous prompts|过往发言)$/u })
    await promptRail.waitFor({ state: 'attached', timeout: 30_000 })
    const promptRailTrack = promptRail.locator('[data-prompt-rail-track]')
    await promptRailTrack.waitFor({ state: 'visible', timeout: 30_000 })
    expect(await promptRailTrack.locator('[data-prompt-rail-mark]').count()).toBe(2)
    expect(await promptRailTrack.locator('[data-prompt-rail-active-dot]:visible').count()).toBe(1)
