...
describe.each(MODE === 'record' ? ['deepseek-official'] : ['deepseek-official', 'feedback-mock'])('web e2e: feedback release for %s', (provider) => {
  ...
  // Price estimates belong to the provider that produced the persisted usage.
  const ackExpected = join(SNAPSHOT_DIR, official ? 'ack.expected.md' : 'ack-mock.expected.md')
  const ackExpandedExpected = join(SNAPSHOT_DIR, official ? 'ack-expanded.expected.md' : 'ack-expanded-mock.expected.md')
  ...
  it.skipIf(MODE === 'record')('keeps the fixture inventory closed', async () => {
    await assertFixtureInventory(SNAPSHOT_DIR, [
      'ack.expected.md', 'ack-expanded.expected.md', 'ack-mock.expected.md',
      'ack-expanded-mock.expected.md', 'feedback-release.expected.json',
    ])
  })
})
