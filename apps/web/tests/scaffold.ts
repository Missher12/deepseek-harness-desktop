...
export async function launchWebScaffold(options: LaunchOptions = {}): Promise<WebScaffold> {
  ...
  const sessionPolicy = composedRows.find(row => row.id === 'session-controller')?.config as {
  ...
  const patches: PatchOptions[] = [
    ...
    ...extraOverlayPatches,
    // Desktop's persistent output root belongs to this fixture, never the developer's home.
    ...sessionPolicy?.noProjectDirectory === undefined ? [] : [{
      id: 'session-controller',
      config: { ...sessionPolicy, noProjectDirectory: join(workspaceCwd, 'deepseek-temp') },
    }],
