...
it('hot-reloads a real client-plugin source edit without refreshing the page', async () => {
  ...
  const originalArtifacts = new Map(await Promise.all(clientArtifactPaths(REPO_ROOT).map(async path => [
    path,
  ...
  try {
    ...
    // A cold two-core hosted runner has measured just under the old 60-second
    // boundary. Give the initial full watch build bounded headroom; the actual
    // source-edit HMR response below retains its strict 30-second deadline.
    await waitForOutput(watcher, /dev-web: watching/, 'pnpm run dev:web', 90_000)
    ...
    const currentArtifacts = clientArtifactPaths(REPO_ROOT)
    await Promise.all(currentArtifacts.filter(path => !originalArtifacts.has(path)).map(async (path) => {
