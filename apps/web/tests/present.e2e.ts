...
`, { mode: 0o700 })
  ...
  it('opens current source files after edits and reload, and reports deletion without downloading', async () => {
    ...
    for (const reload of [false, true]) {
      ...
      expect(await row.getByRole('button', { name: 'Open report.txt', exact: true })
    ...
    if (MODE !== 'record') {
      ...
      const geometry = await page.evaluate(() => {
        ...
        // Literal reasoning also quotes the marker; measure the final reply paragraph.
        const answer = requiredElement(
          [...document.querySelectorAll<HTMLElement>('[data-chat-flow-kind="assistant-step"] p')]
            .find(element => /^PRESENT_DONE\.?$/.test(element.textContent?.trim() ?? ''))
            ?.closest<HTMLElement>('[data-chat-flow-kind="assistant-step"]'),
          'final answer',
        )
        ...
        const open = requiredElement(
          report.querySelector<HTMLButtonElement>('button[aria-label="Open report.txt"]'),
          'report open action',
        )
