...
Desktop 0.4.0 builds the Intel macOS DMG and Windows x64 Setup from the same tested Harness `0.1.1-rc.2` source revision. It also verifies that every Desktop-managed External Brain component is live instead of permanently showing a restart-required status. Windows remains installer-updated; the in-app updater is currently available only for Intel macOS.
