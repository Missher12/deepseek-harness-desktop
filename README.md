...
Desktop 0.4.7 builds the Intel macOS DMG and Windows x64 Setup from the same tested Harness `0.1.1-rc.2` source revision. It adds a non-overlapping 1,600px utility workbench, bounded partial semantic snapshots for complex pages, a visible Agent target marker, longer cold-start recovery, bounded condition waits, composer clearance, and visible Windows installer stages. Both packaged applications must read a real local Accessibility button before release. Windows remains installer-updated; the in-app updater is currently available only for Intel macOS.
